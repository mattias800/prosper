...
uint64_t cpu_rtt_guest_write_bytes(const RttSurf& surface, GuestWriteDrainCensus* census) {
    ...
    return pixels > UINT64_MAX / bpp ? UINT64_MAX : pixels * bpp;
}

RttCache::iterator invalidate_cpu_rtt_entry(
...
void invalidate_cpu_rtt_guest_write(RttCache& cache, uint64_t addr, uint64_t size,
    ...
    if (prepared) {
        ...
        for (auto& entry : *prepared) {
            ...
            invalidate_cpu_rtt_entry(cache, entry.target, effect, census);
    ...
    for (auto it = cache.begin(); it != cache.end();) {
        ...
        it = invalidate_cpu_rtt_entry(cache, it, effect, census);
    }
}
...
void register_live_renderer(const std::string& frame_dir, bool dump_bmps_requested,
    ...
    static const auto g_persist_sampling = [] {
        const char* value = PROSPER_ENV_VALUE("PROSPER_DUMP_PERSISTENT_SAMPLE");
        const auto spec = prosper::frontend::parse_diagnostic_readback_sampling(value);
        const bool master_armed = PROSPER_ENV_ON("PROSPER_DUMP_PERSISTENT");
        if (value) {
            fprintf(stderr, "[persist] sample %s: %s (span=%u stride=%u, "
                            "max 16 sampled callbacks)\n",
                    spec.valid && master_armed ? "armed" : "refused",
                    !master_armed ? "PROSPER_DUMP_PERSISTENT is not armed" :
                    spec.valid ? value : "expected SPAN:STRIDE, span<=256, <=16 samples",
                    spec.span, spec.stride);
        }
        return master_armed || !value ? spec :
            prosper::frontend::DiagnosticReadbackSampling{false, 0, 0};
    }();
    ...
    struct BindProgramTrace {
        ...
        explicit BindProgramTrace(const char* value)
            ...
            if (filter.requested)
                fprintf(stderr, "[bind-program] %s program=0x%llx; "
    ...
    static BindProgramTrace g_bind_program_trace{
        PROSPER_ENV_VALUE("PROSPER_BIND_LOG_PROGRAM")};
              ...
              auto add = [&](const prosper::gpu::ShaderResourceTable* t, uint32_t set,
                ...
                for (auto& r : t->resources) {
                    ...
                    if (image_resource) {
                        ...
                        const bool selected_program = g_bind_program_trace.filter.armed &&
                            set == 1 && draw.fs_guest_addr == g_bind_program_trace.filter.address;
                        if (bind_log || selected_program) {
                            const char* path = has_gpu_live_rtt ? "gpu-bind"
                                : has_uniform_live_rtt ? "uniform-clear"
                                : has_cpu_live_rtt ? "cpu-rtt"
                                : resource_compute_image_hit ? "compute-bind"
                                : decoded_reuse ? "decode-cache" : "decode";
                            const uint64_t at = g_pass_log_submit.load(std::memory_order_relaxed);
                            const uint64_t bl_min = bind_log ? std::strtoull(bind_log, nullptr, 0) : 0;
                            if (bind_log && at >= bl_min && at < bl_min + 3u)
                                fprintf(stderr,
                            ...
                            if (selected_program) {
                                ...
                                if (row <= 8192)
                                    fprintf(stderr,
                                            "[bind-program] render-submit=%d cb=%llu draw=%llu "
                                            "order=%llu ps=0x%llx set=%u binding=%u addr=0x%llx "
                                            "extent=%ux%u fmt=%u tile=%u path=%s "
                                            "rtt_gpu=%d rtt_rgba=%d\n",
                                            g_this_submit, (unsigned long long)at,
                                            (unsigned long long)draw.draw_index,
                                            (unsigned long long)draw.command_order,
                                            (unsigned long long)draw.fs_guest_addr,
                                            set, r.binding, (unsigned long long)r.gpu_addr,
                                            tw, th, (unsigned)r.format, r.tile_mode, path,
                                            live_rtt != g_rtt.end() ? (int)live_rtt->second.gpu_valid : -1,
                                            live_rtt != g_rtt.end() ? (int)(bool)live_rtt->second.rgba : -1);
                                else if (row == 8193)
                ...
                if (PROSPER_ENV_ON("PROSPER_DUMP_PERSISTENT")) {
                    ...
                    if (g_persist_sampling.valid &&
                        ...
                        fprintf(stderr, "[persist] submit=%llu render-submit=%d cb=%llu "
                                "present: front=%d/%d front_va=0x%llx "
                                "selected=%s vo:", (unsigned long long)sub, g_this_submit,
                                (unsigned long long)g_pass_log_submit.load(std::memory_order_relaxed),
                                vo_front, vo_n,
