...
samples at zero on one port -- is counted once, and its duration is reported.
  ...
  usage: audio_queue_timeline_report.py <log> [--min-episode-us N]
...
gate is taken from the sampler's OWN series and needs no second source:
    ...
    a dry sample is an UNDERRUN if a non-zero sample occurs within --active-window-ms on BOTH
    sides of it; otherwise the port was not being fed and the sample is IDLE.
...
def classify_dry_episodes(samples, dry_eps, interval_us, max_dip_us):
    ...
    A dry episode is an UNDERRUN when all three hold:
    ...
    for ep in dry_eps:
        ...
        if fed_before and fed_after and dur_us <= max_dip_us:
            under.append(ep)
        else:
            idle += n
    return under, idle
...
def episodes(samples, is_dry):
    """Maximal runs of consecutive dry samples -> [(start_us, end_us, n_samples)].
...
def report(ports, gaps=None, min_episode_us=0, active_window_us=200000):
    ...
    for port in sorted(ports):
        ...
        all_dry = episodes(samples, lambda q, g: q == 0)
        dry, n_idle = classify_dry_episodes(samples, all_dry, iv, active_window_us)
        thin = episodes(samples, lambda q, g: g and q < g)

        def total_us(eps):
            return sum(n * iv for _, _, n in eps)

        kept = [e for e in dry if e[2] * iv >= min_episode_us]
        print(f"  IDLE (port open, not being fed): {n_idle} samples,"
        ...
        print(f"  UNDERRUN (dry while streaming): {len(kept)} episodes"
              ...
              f", {100.0 * sum(n for _, _, n in kept) / len(samples):.2f}% of samples")
        if kept:
        ...
        print(f"  THIN (under one grain, ungated): {len(thin)} episodes,"
              ...
              f" {100.0 * sum(n for _, _, n in thin) / len(samples):.2f}% of samples")
        ...
        if not kept:
            print("  verdict: the queue never emptied while the port was being fed")
        elif len(kept) < 5:
...
def main(argv):
    ...
    window_us = 200000
    for a in argv[1:]:
        if a.startswith("--min-episode-us="):
            min_ep = int(a.split("=", 1)[1])
        elif a.startswith("--active-window-ms="):
            window_us = int(a.split("=", 1)[1]) * 1000
    if not args:
        print(__doc__)
        return 2
