...
def merge_into_existing_header(existing: str, fresh: str, namespace: str) -> str | None:
    """Splice FRESH's promoted block into EXISTING, keeping what is already there.

    Splitting a large file is iterative -- promote a closure, split, find one more symbol crossing
    the new seam, promote that too -- and the second invocation naturally names the same header,
    because that is where the shared vocabulary already lives. Writing it blind replaced a
    1,021-line header with a 108-line one (#3523).

    Both headers are generated by `build()` and so have the same shape: a preamble, one
    `namespace X {`, a body, and a closing `}  // namespace X`. Merging is therefore splicing two
    bodies inside one namespace, not parsing C++. Returns None when EXISTING is not a header this
    tool wrote, in which case the caller must refuse rather than guess -- overwriting a hand-written
    header would be the same data loss with a different cause.
    """
    open_marker = f"namespace {namespace} {{"
    close_marker = f"}}  // namespace {namespace}"
    if "#pragma once" not in existing or open_marker not in existing:
        return None
    if existing.count(open_marker) != 1 or existing.count(close_marker) != 1:
        return None                       # not the single-namespace shape build() emits
    old_body = existing.split(open_marker, 1)[1].rsplit(close_marker, 1)[0]
    if open_marker not in fresh or close_marker not in fresh:
        return None
    head, rest = fresh.split(open_marker, 1)
    new_body, tail = rest.rsplit(close_marker, 1)
    return (head + open_marker + old_body.rstrip("\n") + "\n"
...
def selftest() -> int:
    ...
    # MERGING INTO AN EXISTING HEADER. The iterative workflow names the same header twice, and the
    # first version wrote it blind -- 1,021 lines replaced by 108 (#3523).
    gen = ("#pragma once\n\n// note\n\nnamespace ns {\n\nint first() { return 1; }\n\n"
           "}  // namespace ns\n")
    fresh = ("#pragma once\n\n// note\n\nnamespace ns {\n\nint second() { return 2; }\n\n"
             "}  // namespace ns\n")
    merged = merge_into_existing_header(gen, fresh, "ns")
    check(merged is not None, "a header this tool wrote is recognised")
    check(merged and "int first()" in merged and "int second()" in merged,
          "both the existing and the new declarations survive the merge")
    check(merged and merged.count("namespace ns {") == 1
          and merged.count("}  // namespace ns") == 1,
          "and the result still has exactly one namespace block")
    # A HAND-WRITTEN header must be refused, not merged: guessing at its shape is the same data
    # loss with a different cause.
    check(merge_into_existing_header("// hand written\nstruct S {};\n", fresh, "ns") is None,
          "a header this tool did not write is refused rather than merged")
    check(merge_into_existing_header(gen.replace("namespace ns {", "namespace other {"), fresh,
                                     "ns") is None,
          "so is one whose namespace does not match")
...
def main() -> int:
    ...
    if args.dry_run:
        dest_preview = source.parent / args.header
        verb = "merge into" if dest_preview.exists() else "write"
        print(f"    would {verb} {args.header} ({header_text.count(chr(10))} new line(s)) and "
              f"shrink {map_data['file']} to {new_source.count(chr(10))} lines")
        return 0

    dest = source.parent / args.header
    if dest.exists():
        # NEVER blind. See merge_into_existing_header: writing over a header a previous promotion
        # produced discards every declaration already lifted there, and the reconstruction check
        # cannot see it because it verifies the SOURCE, not the header.
        merged = merge_into_existing_header(dest.read_text(encoding="utf-8"), header_text,
                                            args.namespace)
        if merged is None:
            sys.exit(f"refused: {args.header} already exists and is not a header this tool wrote "
                     f"(no `#pragma once` plus a single `namespace {args.namespace}` block).\n"
                     f"Overwriting it would discard its contents. Use a different --header, or "
                     f"remove it deliberately first.")
        header_text = merged
        print(f"  [ok]   merged into the existing {args.header}; its declarations are kept")
