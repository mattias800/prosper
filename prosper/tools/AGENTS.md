...
the press). It captures the frames *right after* the press, so it is faithful for persistent glitches and
slowdowns (a single-frame transient could slip by a frame). Replay/debug the window with
`gpu_replay --bundle frame_grab_NNN.prgbundle out.bmp` — the closure is the **last** frame, and its
renderer-owned temporal RTTs (deferred G-buffer / temporal-AA history) are regenerated by the earlier
frames in the window, so it reproduces the real image instead of the black a single submit gives. Drill
into one submit with `--bundle-extract-submit K sub.prgcap`, then `--draw-steps` / `--inspect-only` /
`--dump-resource`. Why a multi-frame window and not one submit: Unity-style titles submit a whole frame
in a single DCB that *samples* RTTs produced by *previous* frames, so one submit replays black — the
window supplies those producers. Raise `PROSPER_CAPTURE_FRAMES` if a title has a deeper temporal history,
or lower it (down to 1) for a lighter capsule when the frame is self-contained.
