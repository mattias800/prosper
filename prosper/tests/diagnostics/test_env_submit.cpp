...
int main() {
    ...
    // correctness" true.
    //
    // The expected value here is TRUE, and that is the whole arm. Site A's cached value at this
    // point is `false` (it sampled a cleared switch inside the block above) while the environment
    // says armed, so the two disagree and only a live read can answer `true`. An arm expecting
    // `false` would be satisfied by the stale cache and would pass whether or not the scope ever
    // closed -- a vacuous arm sitting next to the property it claims to test, which is what this
    // one was until the review caught it.
    //
    // The switch was armed INSIDE that block and is deliberately left as it is; nothing is set
    // here, because a set would be a no-op and describing it as "arming" would overstate what the
    // arm does.
    CHECK(probe_a(), "after the last scope closes, the value armed during it is read live, not "
                     "served from that submit's sample");
