...
int main() {
    ...
    CHECK(reset && setcx && p_add && p_addr && p_dma_src &&
          setcx_direct && setsh_direct && setuc_direct && type2,
          "AGC Dcb functions registered (override the glog stubs)");
    if (!(reset && setcx && p_add && p_addr && p_dma_src &&
          setcx_direct && setsh_direct && setuc_direct && type2)) { printf("== FAIL ==\n"); return 1; }
    ...
    // SDK 3.20's one-dword alignment builder emits a native TYPE-2 filler. Treating it as a
    // successful no-op fails to advance the DCB and changes every following packet offset.
