...
// Build a 4-dword RDNA2 V# (buffer resource): Base48, 14-bit stride @[16:29] of word1,
// num_records=word2, combined 7-bit FORMAT @[18:12] of word3 (PS5 layout — Kyty Shader.h:585).
...
int main() {
    ...
    {
        uint32_t v[4];
        make_vsharp(v, 0x123456780ull, 16, 64, /*fmt*/77);   // 32_32_32_32_FLOAT, stride 16
    ...
    {   // RDNA2 combined-format coverage: the 4 game-observed, Kyty-confirmed anchors + Unknown
        ...
        rdna2_buffer_format(56, &f, &n); CHECK(f == DataFormat::Unorm8 && n == 4, "fmt56 -> Unorm8 x4 (vertex colors)");
        rdna2_buffer_format(22, &f, &n); CHECK(f == DataFormat::Float32 && n == 1, "fmt22 -> Float32 x1");
        rdna2_buffer_format(0, &f, &n);  CHECK(f == DataFormat::Unknown, "fmt0 -> Unknown");
    }
    {   // The game's REAL title-composite vertex-color V# dword3 (0x38fac: dst_sel XYZW + fmt 56)
        uint32_t v[4] = { 0x3c4c7d1cu, 0x0020'7426u, 4, 0x38facu };
        DecodedBufferDescriptor d = decode_buffer_descriptor(v);
        CHECK(d.format == DataFormat::Unorm8 && d.num_components == 4,
              "game color V# (v3=0x38fac) -> Unorm8 x4, not Unknown/Float32");
        CHECK(d.stride == 32, "game color V# stride 32");
    }
    ...
    make_vsharp(&sgprs[4],  0xA0000000ull, 0, 256,  22);   // cbuf0: 256 bytes (stride 0), 32_FLOAT
    ...
    make_vsharp(&sgprs[20], 0xC0000000ull, 12, 90, /*fmt*/74);  // 32_32_32_FLOAT, stride 12
