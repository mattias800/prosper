...
agree with the normative format descriptions and independent decoders.

The audit did find one compiler-portability correctness defect and two misleading declarations:
...
The independent primary implementation was the Khronos CTS decoder at commit
...
[DirectXTex May 2026](https://github.com/microsoft/DirectXTex/releases/tag/may2026) was a second,
vendor-owned cross-check for BC6H/BC7. External code and test data were not copied into prosper.
...
| BC4 | Unsigned endpoint comparison, both ramps, literal 0/1 entries, red-only channel result | Complete ramp block; 3,000 deterministic random blocks matched an independent decoder exactly |
| BC5 | Two independent BC4 streams, channel order, `(R,G,0,1)` result | Complete opposing-ramp block; 3,000 deterministic random blocks matched exactly |
| BC6H UF16 | 14 mode codes, endpoint bit layouts, transformed/direct endpoints, 32 partitions and anchors, 3/4-bit indices, unquantize/interpolate/final-unquantize, reserved modes, HDR-to-RGBA8 clamp | One complete non-trivial known block per valid mode; all 14 outputs matched DirectXTex's BC6H UF16 → RGBA8 conversion exactly |
| BC7 | Eight mode codes, 2/3-subset partitions and anchors, endpoint widths, shared/per-endpoint p-bits, 2/3/4-bit weights, secondary indices, rotation, index selection, reserved mode | One complete non-trivial known block per mode; all eight matched DirectXTex exactly, and 12,000 deterministic random blocks matched a second decoder exactly |

The permanent test contains only fixed block bytes and expected RGBA bytes. Its expected values were
re-derived from the Khronos equations and then cross-checked externally, so CI needs no reference
decoder, downloaded corpus, GPU, game dump, renderer, or snapshot.
...
- BC6H's final conversion to clamped RGBA8 is prosper's existing upload policy, not part of BC6H's
  native HDR representation. The audit checked that the BC6H half-float result and this adapter agree
  with the same conversion in the independent reference path.
