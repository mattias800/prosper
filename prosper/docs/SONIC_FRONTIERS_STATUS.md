...
| The dropped 960x540 draw to `0x20171a0000` is the missing full-screen term | **Not supported by the one experiment that can see it.** Its fragment shader declines on a dynamic `image_load_mip` gated `b.is_compute`; widening that gate behind a throwaway diagnostic made the draw recompile (`[mimg-mip]` declines 1 → 0) and the frame moved **0.933% → 0.929%**, i.e. not at all. Caveat that keeps this OPEN rather than falsified: the diagnostic samples level 0 where the guest asks for level N, so the draw's output is wrong by construction — read the consumer's `nz=` before concluding. **The consumer reading is now taken (2026-09-21), and it does not rescue the hypothesis.** The surface has exactly one consumer in the submit — `draw[70]`, a 1920x1080 three-vertex pass, binding `PS TEX b=38` — and the capture supplies it as a `temporal-RTT-seed` with `captured=4718592 nz=0`: all zeros. So the pass is temporal (it reads its own previous output), and with the producer dropped the history never becomes non-zero, which is consistent with the frame not moving when the producer was forced to recompile for one frame. A single-frame arm cannot fill a history buffer — **if this is re-tested, the producer must be admitted for a run of consecutive frames, not one**, and the arm must read `nz=` on the consumer rather than the composite's non-black percentage. Note also that the 0.933% → 0.929% evidence above predates the world rendering at all; re-measure against a current head before quoting it. #2790. |
