...
`reach-gameplay.pad` route, fresh saves/caches, immediate presentation and serial 380-second
runs. Both arms kept GPU detiling enabled; only `PROSPER_NO_GPU_DETILE_INPUT_REUSE=1` differed.
F8 captured at 300 seconds, before F9 at 330 seconds. The complete captures had no dropped
renderer/compute records. The input mapping now returns to the bounded renderer staging pool
only after the last submission owner releases it; every upload recopies all six source faces.

The other-texture bucket fell from **881.231 ms / 214 references = 4.118 ms/reference** to
**206.440 ms / 233 = 0.886 ms/reference**, a **78.5% reduction in this component per reference**. The captures
recorded 162 and 175 GPU detile uploads, with zero and 175 input reuses respectively. This is
one comparison, not an aggregate FPS result: newly rendered-frame counts were unavailable.
Guest/host presentations were 4.99 and 5.79 per second, and CPU use was 1.88 and 2.03 cores.
Compute cost per record stayed near 1.95 ms; the expensive packed-10-bit sampled-input program
(`0xe3c2229a45c7807c`) still cost 14.30 / 14.58 ms per dispatch. Its conversion/writeback work
remains in #3407.

Both F9 screenshots show the known black Cyber Space world with intact HUD (timers 26.68 and
29.95); they are different guest instants and do not establish a rendered-gameplay milestone.
Before F9, the last cumulative SDL input-demand snapshots showed 1 / 0 shortfall callbacks out
of 15,421 active callbacks in each arm. These are not hardware underruns or evidence of an
audible fix. The separately reproduced opening-cutscene corruption is tracked in #3436.
...
| 2 | bloom/downsample pyramid produced | rgba16f 3840 → 1920 → … → 60 seeds present | correct — but **not the pyramid step 3 reads**. That one is the 2048² mip chain at `0x2026900000`, matched here by SHAPE rather than by address, and it is 32 MiB of zeros. #2790. |
| 3 | ~~**compute `0x2005713000` writes `0x2037960000`**~~ — **the direction is backwards (2026-09-17)**: it **reads** it | the same binding table, read correctly: `class=2` is **Texture**, not a write. `--inspect-only` prints these as `CS TEX b=10 addr=0000002037960000` (the field is `%016llx`, no `0x`), against `CS STORAGE b=4 addr=0000002026900000` for what it does write | ~~never runs~~ — it **did not** run when this row was written (`exact-wave-dispatcher-unsafe guest-barrier=1`); #3713 fixed that and it now executes. `0x2005713000` is a six-level downsampler: it consumes `0x2037960000` through 16 `IMAGE_LOAD`s and writes a mip chain at `0x2026900000`, `0x2026100000`, `0x2025f00000`, `0x2025e80000`, `0x2025e60000`, `0x2025e50000` — **all six zero**. #2790. |
| 4 | **`draw[186]` and `draw[188]` read `0x2037960000`**, write `0x204f9a0000` | their resource lists, both `addr=0x2037960000 declared=33177600` | ~~consumes a surface nothing produced~~ — **stale for the same reason as row 3 (2026-09-17)**. The surface is produced, and it is not empty: `--dump-compute-resource` returns `nz=3,628,800`, and a concurrent lane's retained log shows `[compute-image] code=0x20002fe800 binding=7 class=storage addr=0x2037960000` writing it. What is wrong with it is its **values**, not its existence — it holds exactly two, `0.0` (56.57%) and `0.5` (43.43%). |
| 5 | `0x204f9a0000` is the near-black image | dumped: the same foliage silhouettes as `0x2049a00000`, peak 255, **0.889%** of pixels above threshold 8 | that 0.889% measures **this surface**, not the presented frame, and is not comparable with the 0.933%/0.963% frame figures quoted elsewhere in this doc — see the threshold-spread caveat below |

~~The confirmation that makes edge 3→4 an observation rather than an inference~~ — **this
inference is withdrawn (2026-09-17).** It is true that **`0x2037960000` has
no `rtt-seed` in the capture**, while every other 4K surface in the frame does —
`0x200e230000`, `0x20121f0000`, `0x202b800000`, `0x2049a00000`, `0x204b9e0000`, `0x204f9a0000`,
`0x2068790000`. What does **not** follow is "a surface with no seed is one nothing in the frame
wrote": absence of an `rtt-seed` says the surface is not a renderer-owned render target, not that it
is unwritten. It is captured by another path and carries content — `nz=3,628,800` — and a compute
program writes it. Do not re-derive the missing-producer reading from the seed list.

~~**Read that absence the right way round.**~~ — **withdrawn with the rows above (2026-09-17).**
This paragraph read the missing seed as evidence that `0x2037960000`'s only producer was declined.
The surface is written and carries content, so the premise is gone. **The general caveat it carried
still stands and is the part to keep:** a capture omits *skipped* operations, so "operation X is
absent from the bundle, therefore X is not needed" is never sound on this title — verify replay
coverage first. Apply that to operations, not to this surface.
...
- `0x2005713000` was described as "the final composite that writes the display buffer". The surface
  at issue, `0x2037960000`, is **not** a registered scanout — and note it is this program's **input**,
  not its write target (see the corrected row 3 above; `class=2` is a sampled texture). The registered scanouts are `0x200a160000`
...
| --- | --- |
| The dropped first writer of the HDR buffer (`draw[0]` of submit 138828) is held by a missing opcode, or by the `s_cbranch_vccz` its static scan reports | **Both false.** The inspect header's `first-reject pc=175 fmt=4 op=0x6` is the conservative static scan; in a **fragment** shell a VCC forward-if is not rejected at all (it lowers through `fragment_wave_any`), only the compute shell rejects it. The live terminal reason, reproduced on current main, is `mode=unresolved-operand pc=328 words=8ac47e54` = `s_andn2_b64 s[68:69], s[84:85], EXEC`, and the opcode **is** implemented — what fails is operand resolution. `[graphics-cfg] ... cf_rejected=0` shows the graphics **dispatcher** was not the path taken, so the emission had no scalar-word MUST filter (`rdna2_emit_cfg.cpp:3995-4005`) — which is the operative fact. **It did NOT fall to the straight-line emitter**, and an earlier revision of this row said so wrongly: in the code that name means the `Fs.empty() && Ls.empty()` path (`rdna2_emit_cfg.cpp:7323-7327`), and the `no_placeholders=0` quoted alongside it rules that out, because none of the five sites that clear `scalar_presence_has_no_placeholders` lies on it. `cf_rejected=0` with an `s_cbranch_execnz` in the stream in fact proves the backedge WAS claimed, so the divergent-loop emitter was running. See `RECOMPILER_REMAINING.md` § Ruled out for why widening the compute gate is **not justified by current evidence** — which is not the same as its being demonstrated harmful; the reaching definitions of `s[84:85]` have not been traced yet and they decide it. #2790. |
| `0x2017620000` is a hierarchical luminance reduction whose initial write is the dropped `draw[0]`, so the exposure is computed from stale data | **Falsified — the chain does not exist.** It was inferred from shared addresses and shrinking scissors. Reading what each stage actually fetches: draws 21/23/25 write `0x2017620000` from `0x20171a0000`, a *different* source, and never consume draw 0's output; the reduction is a ping-pong seeded from the HDR at draw 19. The real dependency is one step earlier and more direct — `draw[0]` writes `0x2017620000`, and `draw[1]` fetches it (`PS TEX b=42`, fetch pc=0x15) to write `0x200e230000`, the 4K HDR whose band is the defect. Shared guest addresses and shrinking scissors do not establish a reduction chain; read the bindings. #2790. |
| #3713 (wave64 barrier semantics blocking `0x2005713000`) is the root cause of the black world | **Falsified by fixing it.** The proof was implemented and both `0x2005712000` and `0x2005713000` execute on a **live routed run** with the real renderer — 0 `[compute] skip`, 0 `[compute-cfg-reject]`, 0 device losses — while the frame stays a HUD over black. The verdict rests on those counters against an unchanged picture, **not** on a percentage: the non-black figures quoted across this investigation (0.889%, 0.933%, 0.963%, 0.970%) come from different luminance thresholds and capture paths and are not comparable to each other. Every one of them reads as unchanged. The census skip list went 11 → 9. #3713 is necessary and not sufficient, exactly as the DS `st64` gap was. Do not re-derive it as the cause. #3713 / #3717. |
| The 2048² pyramid at `0x2026900000` is empty because prosper drops `0x2005713000`'s stores (skipped dispatch, lost writeback, or a bad storage view) | **Falsified on the retained bundle, offline.** Prime the destination and re-run the dispatch — `--override-compute-resource 1:4` with 32 MiB of `0x7e`, then `--dump-post-compute-resource 1:4` — and it reports `changed=yes` with 599,040 dwords overwritten, every one to `0x00000000`. The stores execute and land. Positive control on the same path: forcing the **source** to a uniform `1.0f` returns `1.0f` over 2,396,160 dwords, and `0.5f` returns `0.5f` over the region the 1.0f arm wrote, texel for texel (the 0x7e arm's 599,040 dwords is a smaller, strict subset — the shader reads back its own earlier levels, so priming the destination perturbs its input). prosper's load → reduce → store path for this dispatch works; the dispatch is a faithful consumer. #2790. |
| `0x2037960000` is a surface nothing produced (inferred from it having no `rtt-seed`) | **Falsified.** No `rtt-seed` means it is not a renderer-owned render target, not that it is unwritten. It is captured (`captured == footprint`) and carries `nz=3,628,800`, and a concurrent lane's retained log shows `[compute-image] code=0x20002fe800 binding=7 class=storage addr=0x2037960000`. The defect is its **values**, not its absence: over 8,355,840 texels it holds exactly two, `0.0` (56.57%) and `0.5` (43.43%), and a reduction over 56% zeros is what makes every pyramid level zero. The next thing to instrument is `0x20002fe800`. #2790. |
| `[seed-skip-verify]` lines in retained Sonic logs describe a shipped mechanism | **TRUE — and this row previously said the opposite, which was wrong twice over.** `git log -S` settles it: `81a319220` (2026-07-21) added the diagnostic and it shipped on `main` with three emit sites until `389c62dcd` (#3475, 2026-09-08) removed them and flipped `tests/gpu/execute/test_storage_seed_proof_reuse.py:20` from collecting those proofs to forbidding them. So logs from that ~7-week window — which is exactly when `SONIC_FRONTIERS_STATUS.md:878`, `GOLLUM_STATUS.md:16` and `SYBERIA_STATUS.md:23` recorded it — are trustworthy; the mechanism is simply **gone from current `main`** and cannot be re-observed. The two errors worth not repeating: a `git grep` filtered to `*.cpp`/`*.hpp` missed the Python test, and grepping the CURRENT tree was used to conclude a thing had NEVER existed, which no grep of a working tree can establish — that is a `git log -S` question. #2790. |
| The dropped 960x540 draw to `0x20171a0000` is the missing full-screen term | **Not supported by the one experiment that can see it.** Its fragment shader declines on a dynamic `image_load_mip` gated `b.is_compute`; widening that gate behind a throwaway diagnostic made the draw recompile (`[mimg-mip]` declines 1 → 0) and the frame moved **0.933% → 0.929%**, i.e. not at all. Caveat that keeps this OPEN rather than falsified: the diagnostic samples level 0 where the guest asks for level N, so the draw's output is wrong by construction — read the consumer's `nz=` before concluding. **A consumer reading was taken on 2026-09-21 and the conclusion drawn from it is WITHDRAWN (same day).** The observation stands: the surface has exactly one consumer in the submit — `draw[70]`, a 1920x1080 three-vertex pass, binding `PS TEX b=38` — printed as `temporal-RTT-seed` with `captured=4718592 nz=0`. What was inferred from it does not: that the pass reads its own previous output, that it is a history buffer, and that a single-frame arm therefore cannot fill it. **Neither field supports any of that.** `gpu_replay.cpp:578` computes `temporal-RTT-seed` as a pure ADDRESS-MEMBERSHIP test — `any_of(seeds, seed.guest_addr == r.gpu_addr)` — so it says the resource's guest address matches a recorded RTT seed and nothing whatever about previous-frame consumption by the guest; the word "temporal" names the replay's seeding mechanism, not the title's algorithm. And `nz` at `gpu_replay.cpp:555-558` counts non-zero bytes of `r.host_data`, the CAPTURED payload on that table entry — not the image the runtime consumer selects at execution, which a same-frame producer may supersede. This was a game-algorithm claim inferred from a replay-plumbing flag. **To establish the dependency, use command order, resource and subresource identity, and version selection at the consumer** — determine what the producer writes, whether the consumer should observe that same-frame write or an earlier version, what replay actually supplies there, and whether a live producer would supersede the seed. Only if genuine history is demonstrated does a consecutive-frame arm follow. Note also that the 0.933% → 0.929% evidence above predates the world rendering at all; re-measure against a current head before quoting it. #2790. |
...
| The present/composition path is why the stage shows a HUD over black | **Falsified as THE cause, and that is the whole claim.** 40/40 published frames in the stage window are `guest_scanout`, which `guest_scanout_present.hpp:8-17` documents as the correct source for this title — its final composite is not a draw — and an upstream break fully accounts for the picture. (That break was described here as “the consumed 4K surface `0x2037960000` is never produced”; **corrected 2026-09-17** — it is produced, by compute `0x20002fe800`, and carries content. The break is that it holds only two values, `0.0` and `0.5`. The row's verdict is unaffected: the break is still upstream of presentation.) What this row does **not** assert is that presentation is defect-free; it asserts that nothing in presentation needs to change for the world to appear, so do not replace the guest-scanout route to display an intermediate target. #2790. |
| `0x2005713000` writes the display buffer, so forcing its target on screen would show the world | **Falsified, twice over — and note the premise is wrong a third way (2026-09-17): `0x2037960000` is this program's INPUT, not its write target** (`class=2`, a sampled texture; see the corrected row 3 above). It is not a registered scanout — the registered scanouts are `0x200a160000` and `0x200c140000`, both via `RegisterBuffers2` (`[videoout] scanout buffer[N]`, ungated). `0x2037960000` is a 4K intermediate that `draw[186]`/`draw[188]` read. The program is still the right root cause; the "final composite that writes the display buffer" wording was too strong and is retracted. #2790 / #3713. |
| `0x2037960000` is absent from the capture, so the pass that would write it is unnecessary | **Still not a safe inference — but the verdict below is withdrawn (2026-09-17).** This row used to read "Backwards: the missing `rtt-seed` is evidence that the surface's only producer was declined". That conclusion is dead: the surface IS produced (compute `0x20002fe800`, storage binding 7) and carries `nz=3,628,800`, so a missing `rtt-seed` means only that it is not a renderer-owned render target. What survives, and is the reason to keep the row, is the general caution it was built on: **a capture omits *skipped* operations, so "absent from the bundle" never establishes "not needed"** — apply that to operations, not to this surface. #2790. |
...
| `IMAGE_LOAD_MIP` is blocked because the compute backend cannot materialise the guest's mip chain (#2818's stated cause) | **Half falsified — and the surviving half is NOT what the issue says.** The EMITTER's reason for declining was wrong and is fixed (PR #3706); the backend still cannot materialise a chain for THIS resource's shape. Live routed in-stage census 2026-09-17 (`262144 dispatch decisions over 32 program(s)`, 650 stage markers, host quiet at load 0.87 / io 2.9%): all three scene-width kernels report `dyn_shape=1 nsa=0 materialized_mips=12 compute=1` — the address encoding IS recognised and the chain PLAN resolves twelve levels — and the lowering declined anyway. **`materialized_mips` is the PLAN's level count, not a promise the backend will build it:** `compute_binding_mip_chain_materializable` (`live_compute.cpp:12869-12879`) applies a SECOND, independent test the emitter never consults, and it still refuses `img_dim != 1`. This resource is `img_dim=5`, so the dispatch remains declined downstream at `[compute-mip-chain]` (`live_compute.cpp:6766`, whose own diagnostic says "this dispatch is dropped every frame"). Reading `materialized_mips=12` as "the backend materialises it" is the mistake this row exists to stop — it is "a terminal reject line is a lower bound of one", one layer down. **That backend gate is now widened to admit a one-layer 2D_ARRAY (PR #3709), so BOTH blockers are on `main`.** It was not a widening of what the backend can build: `chain_binding_shape`, the image-creation guard, never tested `img_dim` at all — it tests `sampled_layers == 1 && bi.array_layers == 1 && volume_texels == width*height` — so it already accepted a one-layer array and was unreachable only because the dispatch predicate refused first. **The layer bound comes from the PLAN, not from that guard**: every layer term the guard tests derives from the same `r->depth` the plan already pinned, so do not remove `mip_chain_plan.cpp:86`'s `depth != 1` check believing the creation guard covers it. **Neither PR is measured end-to-end** — no routed census has been run against a build carrying both, so "the three scene-width kernels now dispatch" is an expectation, not a result, and the composite baseline is unchanged at 1.003% non-black.** The real blocker was one unprinted gate term, `res->unnormalized` (S# WORD0[15] FORCE_UNNORMALIZED), a **sampler** bit gating an operation that binds no sampler and addresses texels by integer coordinate. The sibling zero-mip path never consulted it, so the two `IMAGE_LOAD_MIP` routes disagreed. Fixed in PR #3706, which also extends `[mimg-mip]` with `ord2d`/`res_unnorm`/`depth_cmp`/`dim_is_2d` so the line can no longer report "declined" while every field it prints looks satisfied. Note `res_unnorm` is the RESOURCE's field; the pre-existing `unorm=` is the INSTRUCTION's `mimg_unorm` and is a different thing. #2818 |
| The title-screen heading's wrong artwork is the persistent-colour-target borrow path serving a retained render target | **Falsified.** `PROSPER_MIPLOG=1` on the replayed title-screen capsule reports that binding as `1024x1024x1 dim=5 declared_mips=1 fmt=37 **rtt=0** ds=0 rgba=1` — no `persistent_render_target_id`, so `find_persistent_color_target` is never consulted. That lookup is also extent-exact (its cache key is `{id,width,height,format}`, `render_runner.h:2570`), so it would have missed a 504x204 target on a 1024x1024 query anyway. With `rtt=0` the sampler falls back to `tex_rgba`, i.e. **guest memory bytes**. #2206 |
| Address aliasing between a sampled texture and a UI render target is itself the #2206 defect | **Falsified — aliasing is the NORMAL mechanism here.** In one title-screen submit, six of eight UI elements sample a texture at the same guest address as a render target prosper holds, and all six render correctly *because their extents agree*: `0x2061750000` 384x60, `0x2085410000` 244x36, `0x2085db0000` 384x60, `0x208d360000` 1028x36, `0x208d3f0000` 384x60, `0x208d450000` 384x60. Exactly two **disagree**, and exactly those two are wrong: `0x20869e0000` (T# 1024x1024 vs target 504x204, content "Try Again", and the only non-identity swizzle in the submit, `swz=6547` = Z,Y,X,W, which is what turns the target's `(64,128,160)` blue into the frame's `(160,128,64)` gold) and `0x20853d0000` (T# 512x256 vs target 132x36, content "Loading"). Both wrong elements hold a transient string from a *different* screen. So the discriminator is the extent disagreement, not the aliasing — and the six matching elements are a built-in control a fix must not regress. #2206, #3436 |
| A retained F9 `.prgbundle` can be used to debug the stage's skipped compute programs offline | **Falsified.** A capture records what RAN, and these programs are precisely the ones that did not. Replaying the 2026-09-14 black-world gameplay bundle (1.4 GB) with `--recompile-raw` on a build carrying an extended `[mimg-mip]` diagnostic runs to its `closure` line and prints **zero** `[mimg-mip]` lines; most submits report `operations=0/0`. The offline bundle loop IS excellent for this title's *rendered-frame* defects — it gives #2206 a deterministic seconds-long repro where the routed run takes ~5 minutes and desyncs under load — but the compute census requires a routed arm. #2790, #2818 |
| A routed census arm is safe to start whenever CPU is free | **Falsified, and the failure is silent.** An arm taken at load 28 returned `131072 dispatch decisions over 5 program(s)` with **0** stage markers — never left the menus. In the same sample `/proc/pressure/cpu` read `some avg10=0.00` while `/proc/pressure/io` read `some avg10=90`: the load average was **entirely I/O wait** (another lane was running 24 concurrent `ld` processes; prosper's test binaries are 100-125 MB each). This boot's progression is paced by **asset loading**, which is I/O, while the route is flip-anchored — so free CPU is not sufficient. Check `/proc/pressure/io`, not `uptime` or CPU. #2790 |
| The black world is explained by the three scene-width compute passes not executing | **FALSIFIED — and this is the strongest of the five instances, because the census DID move this time.** With BOTH #2818 blockers merged (#3706, the recompiler declining on a sampler bit that cannot describe a no-sampler op; #3709, the backend refusing a one-layer 2D_ARRAY chain), a routed in-stage census on a quiet box (`262144 dispatch decisions over 32 program(s)`, 300 stage markers, io 11-14%) shows `0x2005714000`, `0x2005717e00` and `0x200571bd00` — **all three 2880-thread-wide screen-space passes** — with **zero mentions in the run log**, i.e. no longer skipping at all, with `[mimg-mip]` 0 and `[compute-mip-chain]` 0. Programs at `executed=0` fell 16 -> 14 -> **11**. A live 40-sample `tools/screenshot` arm on the same build (2,399 s, guest flip 23,143, no warmup, io 4% at start) then measured the composite: **38 of 40 in-stage samples at 0.880-1.096% non-black with bbox `x[64..3776] y[59..2090]` on EVERY one** — identical to the pre-fix baseline of 1.003% / the same bbox. The guest is genuinely playing (rings climb to 043/999, stage clock 09:53.04). The previous four instances could be explained by "another decline was waiting behind it"; **this one cannot, because for these three there is no decline left.** So whatever keeps the world black is among the remaining 11 programs or is not a compute-execution problem at all. The outcome was predicted publicly on #2818 before the arm ran. #2790, #2818 |
| The Cyber Space world is not rendered (#2790's premise, and this document's own wording) | **FALSIFIED — prosper renders it correctly; it never reaches the guest's display buffer.** An F9 bundle captured in-stage on 2026-09-17 (1.66 GB, quiet box, io 2%) replays to the same HUD-over-black frame offline (0.954% non-black, bbox `x[64..3776] y[59..2090]`), so the defect is reproducible without a route. Its largest submit carries **189 draws, 167 of them targeting 3840x2160**, plus a full rgba16f downsample pyramid (3840->1920->960->480->240->120->60) — an ordinary HDR post chain. Dumping the frame's 4K render targets (`--dump-rtt-seed`) finds **four with substantial content, every one bounded to exactly `x[0..2879] y[0..1619]`** — the 2880x1620 75% dynamic-resolution rect this document already identified: `0x200e230000` 53.9% non-black (mean lum 125.3), `0x202b800000` 56.3%, `0x204b9e0000` 52.0%, `0x2049a00000` 39.6%. **`0x2049a00000` is a fully textured, correctly lit Cyber Space level** — checkered ground, the orange checkered wall, palm trees, flowers, red dash panels, rails (`assets/screenshots/sonic-frontiers-cyberspace-world-in-target.webp`). So the renderer is not the frontier: the frontier is why that content does not reach the display buffer the guest composites and prosper publishes. Note `guest_scanout` is **correct** for this title (40/40 samples; `guest_scanout_present.hpp:8-17` — its final composite is not a draw, so prosper has no present source of its own), so the present path is exonerated too. **Instrument note:** replaying an EXTRACTED submit standalone reports `Vulkan render FAILED` and writes 0 bytes — that is the absent guest presentation surface (`test_draw_program_skip_render.cpp:114-118`), apparatus rather than defect; use the full-bundle replay. #2790, #1891 |
...
| Admitting the layered compute dispatches this backend skips will change the frame | **Not established, and it did not.** The `layered image deferred to #657` skip refused two bindings in the Cyber Space frame, and three dispatches behind them reported `result=failed` (`0x2005a11600`, `0x200581bb00`, `0x2005a13f00`). Admitting the cube-sampled and query-only-1D shapes takes the same F9 bundle from 2 skipped bindings / 38 of 41 `result=ok` to **1 skipped / 40 of 41 ok** -- and every one of the **46 per-submit output hashes is byte-identical to `origin/main`**, 41 of them full 3840x2160x4 colour readbacks. So the admission is real and the picture is unmoved: this is the fifth time an unblocked compute program has left this frame unchanged, and it removes compute admission from the suspect list for the remaining world defect rather than advancing it. Scope: one frame, replayed. The remaining skip (`0x200581bb00`, a cube bound as 2D-array STORAGE) is deliberately still refused -- its writeback would publish one of six faces, which is worse than a loud skip (#3742) -- and none of this clears the other programs still `executed=0` for different reasons. #657, #2790 |
| The bloom/downsample mip chain is what blows the frame out | **No, and it is cheaply falsified.** The 2026-09-20 offline replay leaves all six bloom levels (`0x2026900000` .. `0x2025e50000`) at `stop=unresolved-producer producer=0`, i.e. **zero** — and the replayed frame still shows the identical three-band structure as the live grab. Row-luminance, top to bottom: live `[8, 9, 11, 148, 250, 254, 255, 255, 251, 192, 157, 121, 111]`, replay `[7, 5, 2, 16, 191, 246, 255, 241, 181, 152, 121, 115, 109]`. **Scope, stated precisely because the arms are not identical:** this establishes that a saturated band arises with the bloom chain at zero, so bloom is **not necessary** for the defect and is not where to start. It does not prove bloom contributes nothing to the live frame, whose near-white is 42.31% against the replay's 27.69% — six producers differ between the arms, so some of that gap may be bloom. What is excluded is bloom as *the* cause. Recorded because the blurred edges of the white band make bloom the obvious first guess, and because the falsifying arm is a by-product of the replay's closure rather than a deliberate lever — if you want the stronger result, override the six levels explicitly on a live run. #2790 |
| The F9 bundle cannot reproduce this frame's composite, so the defect must be chased live | **Half true, and the useful half is the other one.** The replay's absolute values do differ from the live grab (78.00%/27.69% non-black/near-white against 84.31%/42.31%), because six producers are unresolved. But it reproduces the **defect's structure** exactly — same three bands, same saturated middle — so it IS the fast loop for this bug. Extracting the one submit is a faithful standalone repro: `gpu_replay --bundle <f> --bundle-extract-submit 138828 s.prgcap` then replaying `s.prgcap` scores **44.03%/15.62%**, identical to `--bundle-through-submit 138828`. Use the bundle for cause, the live grab for absolute numbers. #2790 |
| The blow-out is a global exposure error, so the whole frame is uniformly bright | **No — it enters at ONE submit and is banded.** A through-submit sweep is clean: near-white is **0.00%** through 138818, 138820 and 138824, **15.62%** at 138828, **27.69%** at 138830. And the frame is three bands, not a uniform lift: black top ~28% where the oracle has sky and palm trees, a saturated band with **soft blurred edges** through the middle, flat grey-blue bottom ~25% where the oracle has textured grass, and a **correct HUD** (ring counter, timer, speedometer) over all of it. Submit 138828 is the post-process chain: a bloom pyramid at draws 7-17 (1920 -> 60 -> 1920) and an adaptation path whose 2x1 `rgba16f` targets (`0x2063780000`, `0x2059ac0000`) are written by draws 26-27 and read by draw[25] as `b=35`. **Not yet established which draw in 138828 introduces it** — note that `--draw-steps` and `--draw 0:N` both dump whichever target that range ends on, so neither is a valid composite-over-time bisect; `--output-target-after OP:ADDR` against one fixed address is. #2790 |
