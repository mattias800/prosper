...
| Admitting the layered compute dispatches this backend skips will change the frame | **Not established, and it did not.** The `layered image deferred to #657` skip refused two bindings in the Cyber Space frame, and the three dispatches behind them reported `result=failed` (`0x2005a11600`, `0x200581bb00`, `0x2005a13f00`). Admitting all three takes the same F9 bundle from 2 skipped bindings / 38 of 41 `result=ok` to **0 skipped / 41 of 41 ok** -- and every one of the **46 per-submit output hashes is byte-identical to `origin/main`**, 41 of them full 3840x2160x4 colour readbacks. So the admission is real and the picture is unmoved: this is the fifth time an unblocked compute program has left this frame unchanged, and it removes compute admission from the suspect list for the remaining world defect rather than advancing it. Scope: one frame, replayed; it does not clear the *other* thirteen programs that are still `executed=0` for different reasons. #657, #2790 |
