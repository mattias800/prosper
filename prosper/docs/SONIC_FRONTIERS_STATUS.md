...
| `[seed-skip-verify]` lines in retained Sonic logs describe a shipped mechanism | **No code emits it — it names a REMOVED mechanism that is now actively forbidden.** `prosper/tests/gpu/execute/test_storage_seed_proof_reuse.py:20` asserts no output line may start with `[seed-skip-verify] `, on the grounds that "destructive historical coverage proving must not run". (An earlier version of this row said a whole-tree grep found the string "only in prose"; that was wrong, and wrong by the very scoping mistake the row goes on to warn about — the grep was filtered to `*.cpp`/`*.hpp` and so skipped the Python test.) It was a temporary instrument in another lane's worktree, never merged, and cannot be reproduced. Three neighbouring diagnostics in the same logs — `[compute-image]`, `[compute-image-writeback]`, `PROSPER_COMPUTE_IMAGE_TIMING` — **are** real, but live under `prosper/frontends/`, so a `grep` scoped to `prosper/src/` calls all three absent. Search the whole tree. #2790. |
