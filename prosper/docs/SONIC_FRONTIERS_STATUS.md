...
| 3 | **compute `0x2005713000` writes `0x2037960000`** (4K, 33,177,600 B) | its binding table: bindings 10, 12–18, 20–23, 25–28 all `class=2 addr=0x2037960000` | ~~never runs~~ — it **did not** run when this row was written (`exact-wave-dispatcher-unsafe guest-barrier=1`); #3713 fixed that and it now executes, with `0x2037960000` reading `nz=3628800`. The row is kept because steps 1–5 are the chain as measured at the time; see the strike below and #2790's later comments. |
