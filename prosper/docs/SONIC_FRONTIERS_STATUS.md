...
| 3 | ~~**compute `0x2005713000` writes `0x2037960000`**~~ — **the direction is backwards (2026-09-17)**: it **reads** it | the same binding table, read correctly: `class=2` is **Texture**, not a write. `--inspect-only` prints these as `CS TEX b=10 addr=0x2037960000`, against `CS STORAGE b=4 addr=0x2026900000` for what it does write | ~~never runs~~ — it **did not** run when this row was written (`exact-wave-dispatcher-unsafe guest-barrier=1`); #3713 fixed that and it now executes. `0x2005713000` is a six-level downsampler: it consumes `0x2037960000` through 16 `IMAGE_LOAD`s and writes a mip chain at `0x2026900000`, `0x2026100000`, `0x2025f00000`, `0x2025e80000`, `0x2025e60000`, `0x2025e50000` — **all six zero**. #2790. |
...
| `[seed-skip-verify]` lines in retained Sonic logs describe a shipped mechanism | **No such diagnostic exists on any branch** (`git grep` over `*.cpp`/`*.hpp` finds it only in prose). It was a temporary instrument in another lane's worktree, never merged, and cannot be reproduced. Three neighbouring diagnostics in the same logs — `[compute-image]`, `[compute-image-writeback]`, `PROSPER_COMPUTE_IMAGE_TIMING` — **are** real, but live under `prosper/frontends/`, so a `grep` scoped to `prosper/src/` calls all three absent. Search the whole tree. #2790. |
