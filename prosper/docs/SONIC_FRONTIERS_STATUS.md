...
| --- | --- |
| The present/composition path is why the stage shows a HUD over black | **Falsified.** 40/40 published frames in the stage window are `guest_scanout`, which `guest_scanout_present.hpp:8-17` documents as the correct source for this title — its final composite is not a draw. The break is upstream: the consumed 4K surface `0x2037960000` is never produced. Do not replace the guest-scanout route to display an intermediate target. #2790. |
