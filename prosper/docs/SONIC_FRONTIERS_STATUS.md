...
| `IMAGE_LOAD_MIP` is blocked because the compute backend cannot materialise the guest's mip chain (#2818's stated cause) | **Falsified.** Since #3197/#3237 it materialises the whole chain. Live routed in-stage census 2026-09-17 (`262144 dispatch decisions over 32 program(s)`, 650 stage markers, host quiet at load 0.87 / io 2.9%): all three scene-width kernels report `dyn_shape=1 nsa=0 materialized_mips=12 compute=1` — the address encoding IS recognised and **all twelve levels ARE materialised** — and the lowering declined anyway. The real blocker was one unprinted gate term, `res->unnormalized` (S# WORD0[15] FORCE_UNNORMALIZED), a **sampler** bit gating an operation that binds no sampler and addresses texels by integer coordinate. The sibling zero-mip path never consulted it, so the two `IMAGE_LOAD_MIP` routes disagreed. Fixed in PR #3706, which also extends `[mimg-mip]` with `ord2d`/`res_unnorm`/`depth_cmp`/`dim_is_2d` so the line can no longer report "declined" while every field it prints looks satisfied. Note `res_unnorm` is the RESOURCE's field; the pre-existing `unorm=` is the INSTRUCTION's `mimg_unorm` and is a different thing. #2818 |
