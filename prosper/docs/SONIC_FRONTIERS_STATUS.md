...
| `IMAGE_LOAD_MIP` is blocked because the compute backend cannot materialise the guest's mip chain (#2818's stated cause) | **Half falsified — and the surviving half is NOT what the issue says.** The EMITTER's reason for declining was wrong and is fixed (PR #3706); the backend still cannot materialise a chain for THIS resource's shape. Live routed in-stage census 2026-09-17 (`262144 dispatch decisions over 32 program(s)`, 650 stage markers, host quiet at load 0.87 / io 2.9%): all three scene-width kernels report `dyn_shape=1 nsa=0 materialized_mips=12 compute=1` — the address encoding IS recognised and the chain PLAN resolves twelve levels — and the lowering declined anyway. **`materialized_mips` is the PLAN's level count, not a promise the backend will build it:** `compute_binding_mip_chain_materializable` (`live_compute.cpp:12869-12879`) applies a SECOND, independent test the emitter never consults, and it still refuses `img_dim != 1`. This resource is `img_dim=5`, so the dispatch remains declined downstream at `[compute-mip-chain]` (`live_compute.cpp:6766`, whose own diagnostic says "this dispatch is dropped every frame"). Reading `materialized_mips=12` as "the backend materialises it" is the mistake this row exists to stop — it is "a terminal reject line is a lower bound of one", one layer down. **The remaining #2818 work is extending that backend gate to `img_dim == 5 && depth == 1`, with its own evidence.** The real blocker was one unprinted gate term, `res->unnormalized` (S# WORD0[15] FORCE_UNNORMALIZED), a **sampler** bit gating an operation that binds no sampler and addresses texels by integer coordinate. The sibling zero-mip path never consulted it, so the two `IMAGE_LOAD_MIP` routes disagreed. Fixed in PR #3706, which also extends `[mimg-mip]` with `ord2d`/`res_unnorm`/`depth_cmp`/`dim_is_2d` so the line can no longer report "declined" while every field it prints looks satisfied. Note `res_unnorm` is the RESOURCE's field; the pre-existing `unorm=` is the INSTRUCTION's `mimg_unorm` and is a different thing. #2818 |
