...
|---|---|---|
| There is no width-aware scalar **read** enumerator, so proving "no data-domain read of register R" requires writing one | **False, and this premise drove a design to the wrong mechanism.** `sgpr_dead_at_merge` (`rdna2_cfg_support.hpp:84-280`) is a per-format, CFG-walking scalar read scanner: implicit VCC readers, both successors of every branch, SMEM SBASE/SOFFSET, MIMG descriptor ranges, MUBUF SRSRC, FLAT SADDR, implicit destination RMW, and a fail-closed `return false` default for anything it cannot bound. The per-source width table is `scalar_alu_source_words` (`:1295`), and `scalar_implicit_destination_read_width` (`:55`) covers the implicit side. Believing it absent led to preferring a runtime `operand_bits` guard over a static census -- the wrong direction, because a miss in a census over-rejects while a miss in the guard fabricates. | #2790 |
| A register absent from `sreg_bool` / `sreg` / `sreg_input` / `sreg_bool_b32` at a loop preheader has no reaching definition | **No -- absence is not a reaching-definition oracle, and the codebase says so in three places.** `rdna2_to_spirv_internal.hpp:2681-2683` ("an unrepresentable write deliberately erases its SSA value", which is why `sreg_written` exists) and `:2993-2995` ("folded/data writers such as `s_getpc_b64` intentionally leave no scalar value behind, so absence from `sreg` cannot identify a mask write"). Concretely: `rdna2_emit_cfg.cpp:7520-7527` erases from `sreg_bool`/`sreg_bool_b32` every mask created inside a loop because its SSA id does not dominate the merge, so a mask defined in one loop and used as an accumulator in the next is absent from all four maps **while a value genuinely reaches**. Two further maps hold definitions such a check would not consult (`sreg_wave64_mask_half`, `sreg_entry_m0` -- the latter *defined by* absence). In a fragment stage `sreg_input` is never populated at all (`rdna2_recompile_compute.cpp:737` is compute-only), so the test degenerates to "this compile has not written it yet". Single entry into `emit_divloop` is real but does not supply the property. | #2790 |
| `emit_divloop` leaves a scalar register untouched in `rs.sreg` when the loop only writes it as a 64-bit mask | **No.** `:7399` creates a u32 placeholder phi for every register in `cs`, and `cs` comes from `loop_written_regs` (`:1129`), whose SOP2 case inserts the destination of `s_or_b64`. So a mask accumulator IS present in `rs.sreg` as a fabricated zero from the preheader onward -- which is exactly what the `[wave-mask-unresolved]` probe reports as `sreg=1/1`. Any guard of the form `marked && !rs.sreg.contains(r)`, copied from the entry-M0 token, is therefore **dead on this shape**: the M0 clause is correct only because a later real write supersedes the token, and here there is no later real write, only the emitter's own placeholder. | #2790 |
| Dropping the `b.is_compute` gate on the wave-mask scalar-pair projection (`rdna2_emit_alu.cpp`'s `mask()`) is the repair for the fragment shaders it rejects | **Not justified by current evidence** — which is weaker than "wrong", and the distinction is the point. The stage predicate is *not* the obstacle: fragment lane identity is sound (`guest_lane_id()` returns `subgroup_local_id()` for fragment, the backend enforces an exact 64-lane fragment subgroup via `requiredSubgroupSize`, and the sibling `inline_int_mask_bit` clause already performs the identical projection in fragment). What is missing is a **definite-assignment proof for the operand**, which the non-dispatcher emission path does not have: it seeds absent SGPRs with a fabricated `uconst(0)` at loop phis and if-merges and then clears `scalar_presence_has_no_placeholders`, while the dispatcher MUST-filters `state.sreg` (`rdna2_emit_cfg.cpp:3995-4005`). **Do not call that path "straight-line"** — an earlier revision of this row did, and the name is taken: it means the `Fs.empty() && Ls.empty()` path (`:7323-7327`), which has neither loop phis nor if-merges and cannot be what a `no_placeholders=0` observation came from. `mask()`'s fallback consults neither. Measured on *Sonic Frontiers*' dropped HDR producer (`s_andn2_b64 s[68:69], s[84:85], EXEC` at pc=328, fragment): `sreg_bool=0 sreg=1/1 sreg_input=0/0 no_placeholders=0 wave=64 native_sg=0`. **Read that precisely.** It shows no tracked predicate for this operand here, both scalar entries present, and that membership is not a definite-assignment proof in this emitter state. It does **not** show that either entry is a fabricated zero, that projecting the pair yields zero, or that the source was never EXEC/VCC — `no_placeholders` is a broad state property that cannot name a register, predecessor or dynamic path, and a missing `sreg_bool` entry is equally compatible with lost mask provenance. Widening therefore lacks a basis, and so does declaring the reject correct; the reaching definitions of s84 and s85 decide it. | #2790 |
| The V# `DST_SEL` channel swizzle is "a FORMAT-fetch control", so it binds on every typed fetch -- MTBUF `tbuffer_load_format_*` included -- and `rdna2_emit_alu.cpp`'s "MTBUF forces identity selection" comments contradict `shader_resources.cpp:210` | **Falsified for MTBUF; confirmed for MUBUF. The two comments were both right, of different instruction families, which is why neither reader could reconcile them.** RDNA2 ISA (document 70648) sec. 8.1.4 "Buffer Data" says outright *"Dst_sel comes from the resource, but is ignored for many operations"*, and its Table 31 gives the whole partition: `BUFFER_LOAD_FORMAT_*` / `BUFFER_STORE_FORMAT_*` -> DST SEL **resource**; `TBUFFER_LOAD_FORMAT_*` / `TBUFFER_STORE_FORMAT_*` -> **identity**; `BUFFER_LOAD_<type>` / `BUFFER_STORE_<type>` / `BUFFER_ATOMIC_*` -> **identity** -- where identity is defined in the same table as "X000, XY00, XYZ0, or XYZW" by the data format's component count, i.e. a SUBSTITUTION that zeroes absent channels, not a pass-through. So applying the descriptor's routing to MTBUF would have been a new defect wearing the fix's clothes; the test that pins it is the MTBUF arm asserting the routing is IGNORED. Corroborated behaviourally by Mesa, which carries the BGRA vertex swizzle in the descriptor's `DST_SEL` on its `buffer_load_format_xyzw` path and hand-reverses the per-channel byte offset in the shader on its `tbuffer_load_format_*` path (`aco_select_vs_prolog.cpp`, `radv_cmd_buffer.c` `post_shuffle`) -- a driver would not pay for that if MTBUF honoured the field. **Not settled by the document: the DIRECTION of the mapping for `BUFFER_STORE_FORMAT_*`, and what a constant selector means on the way out.** A routed format store therefore fails visibly rather than guessing. | #2869, PR (this) |
| A PS5 RectList that renders as a single triangle is being **declined** post-VS fourth-corner synthesis, because `needs_rect_list_synthesis` requires a vertex buffer and a full-screen movie blit is vertex-less | **Falsified on both halves.** A census over Darksiders II's intro counted **7,200 RectList draws: 2,397 synthesized, 4,803 expanded by the procedural `vb_entries == 0` path, and ZERO unhandled** -- the vertex-buffer precondition never declined a draw, and the declines it did see were `vertex_resources == nullptr` (the shader has no resources in any user-data base), which reaches the *other* path and is handled. The blit is also not vertex-less: it carries two vertex buffers, stride 16, three records. The real defect was **inside** synthesis -- the geometry stage assumed the shared corner is always P0 (fourth = `P1 + P2 - P0`, emitted in the supplied order) while this draw shares at P2, so the synthesized corner landed off-screen at clip (-1,3). Read the census before blaming the precondition; a symptom that is half a quad does not locate the half that is missing. | #3507, PR #3511 |
| A fragment module whose wave-reason set is exactly `WaveAny` is **width-agnostic**, so it can run at the host's native 32-lane fragment subgroup because its vote is "control flow only" | **Falsified.** The reason set records how the vote was PRODUCED, not what it can do, and "control flow only" is not the harmless property it sounds like: a vote used purely as a branch condition decides WHICH store runs. With the guest predicate true in the upper 32 lanes and false in the lower, a 64-lane `any()` takes a branch that two independent 32-lane groups disagree about, and whatever the taken block writes lands in one half only. Measured over 49 real fragment modules from four titles, **48 of 49** have a vote deciding a selection whose merge carries different values into a colour output. The admission needs a property of the MODULE, not of the vote's provenance. | #3464, PR #3480 |
| That property is **value reachability**: admit a module when the vote's value cannot reach a colour `Output` store | **Falsified, and by the widest possible margin.** Value reachability is a data-flow question and the mechanism above is a control-flow one, so the two barely intersect: over the same 49 modules, value reachability clears **49 of 49** while control dependence clears **1**. A predicate that admits everything is not a conservative approximation of one that admits almost nothing. The predicate that ships instead (`fragment_spirv_wave_width_independent`) clears a vote on either of two grounds -- its operand is provably wave-uniform, so `Any(P) == P` at any width; or its result cannot influence an output through data flow OR control dependence -- and clears **38 of 49**. | #3464, PR #3480 |
...
| `SignedZeroInfNanPreserve` needs no device gate, because `shaderSignedZeroInfNanPreserveFloat32` is VK_TRUE on every implementation prosper runs on -- and a device that ever reported VK_FALSE would make **pipeline creation fail loudly** rather than mis-evaluate silently | **Both halves falsified, by CI, on the first run after the unconditional declaration landed.** Declaring the capability without satisfying the device requirement does not fail: it makes the module *invalid*, and drivers honour an invalid module. The Vulkan validation scan reported `VUID-VkShaderModuleCreateInfo-pCode-08740` (the property must be VK_TRUE) and `-08742` (Vulkan 1.2 or `VK_KHR_shader_float_controls` required) **x475 each** across `cfg_trip_bound_exec`, `rdna2_to_spirv_exec`, `storage_image_copy` and `traversal_chase_exec` -- **while all 464 tests passed**. Nothing in the tree queried the property (0 hits for `FloatControls\|shaderSignedZeroInfNanPreserve`, against a positive control showing `vkGetPhysicalDeviceProperties2` IS used for subgroup properties at `tests/fixtures/render_runner.h`), and prosper creates Vulkan **1.1** devices in its own test fixtures, so the second requirement was unmet by prosper's own code rather than by some hypothetical device. Same shape as #2422: an undefined contract a driver happened to answer the expected way. The declaration is now published from a measured device (`src/gpu/execute/float_controls_probe.hpp`) and defaults to absent; `test_float_controls` pins both directions. **The general lesson, and the reason this row exists rather than just a code comment: a device-capability claim that no code queries cannot be contradicted by any green test run**, so "every device we run on has it" is not evidence, it is an assumption wearing evidence's clothes. | #3479, #3561 |
...
| The wave-width-independence analysis' observable-effect predicate is a **memory-WRITE** test, so every opcode on it mutates memory and `OpAtomicLoad` sitting there is simply a bug | **Half falsified, and the half that survives is the half that matters.** `OpAtomicLoad` really does not write -- SPIR-V's unified grammar gives it the only atomic signature with **no `Value` operand** (`IdResultType IdResult Pointer Memory Semantics`), and the spec forbids `Release`/`AcquireRelease` on it while forbidding `Acquire`/`AcquireRelease` on `OpAtomicStore`, which is the publishing half being denied to a read. But `is_observable_effect` has **two** callers and only one asks about writing: arm (b)'s control-dependence scan uses it to ask whether a vote-guarded region did anything the rest of the draw can see, and a barrier -- which writes nothing at all -- has been on it since the Evergate measurement. So the predicate is about **touching/ordering**, `writes_memory` is it minus an explicit writes-nothing list, and the fix belongs in that subtraction. Removing `OpAtomicLoad` from the predicate itself instead **admits a vote-guarded region containing one**, measured: the `region_atomic(227)` arm flips from NOT PROVEN to independent. Do not "simplify" the two lists back into one. | #3504, PR #3547 |
| The const-fold's branch-exclusive `FoldStateSnapshot` storage is what puts `memmove` (6.40%) and `memset` (2.59%) above every prosper function in the render-thread profile -- one `std::vector<std::pair<bool, FoldStateSnapshot>>` value-initialised per fold call, at **16,504 bytes** per slot | **Falsified by counting how often the branch is taken, not how big the slot is.** New `PROSPER_STAGE_FOLD_PROFILE` counters (slots reserved against captures taken) over a 400,000-call routed *Grand Theft Auto V* window: **19,513 slots in total, 0.0488 per call**. That mean is not itself a fraction of calls -- the counter accumulates a call's whole `snapshot_count`, which can exceed one -- but every call that reserves anything reserves at least one slot, so at most 19,513 of the 400,000 calls reserved any: **at least 95.1% reserved none**. In bytes, 19,513 x 16,504 = 322 MB of value-initialisation across the window, **805 bytes per call**, against a fold body the same profiler reports as 1096.16 ms over those 400,000 calls, i.e. **2.74 us per call**; and an after-profile with slot pooling in place was not lower. No per-call timing of the snapshot path itself was taken, so the falsification rests on that frequency bound and the null A/B rather than on a measured time share. The pooling was reverted; the counters that produced the negative are kept, so this is re-checkable rather than re-derivable. The `sizeof` is real on x86-64 Linux (measured by a deliberate template-probe compile error) and is **16,488** under libc++, where a `std::unordered_map` member is 16 bytes narrower -- it is a property of the ABI, so do not hard-code it; a `static_assert` pinning 16,504 failed the macOS build of this change's first revision. The arithmetic was right and the frequency was never measured | #3407 |
