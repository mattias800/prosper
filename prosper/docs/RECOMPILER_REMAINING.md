...
| A fragment shader whose only wave-width reason is \`OpGroupNonUniformAny\` (a branch-guard vote) **may be width-agnostic**, so a narrow device (NVIDIA 32..32, llvmpipe 8..8) can run it at its native width instead of skipping the draw | **Rejected — the third attempt (#2404, #2410, and a 2026-08-25 PR), each caught by the same two arms.** The predicate is unsound: \`fragment_wave_any\`'s result is routinely consumed as guest scalar **data** via SCC (\`s_cselect_b32\`, \`s_addc_u32\`, \`s_subb_u32\` — five assignment sites in rdna2_emit_alu.cpp), so pinning 32 makes half-wave shading depend on which half holds a passing lane. Even a proven guard is insufficient: a guarded block's emulated scalar state goes **stale** in the non-entering half-wave after the merge (DOLL's FXAA shape, #273). #2414: "the reclassification premise is dead". The fail-visible skip is the correct behavior until a shape-level dataflow analysis exists (#2410's criterion). The per-call-site label cannot express it. | #2404, #2410, #2414, #2985 |
