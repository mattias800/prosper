...
| The const-fold's branch-exclusive `FoldStateSnapshot` storage is what puts `memmove` (6.40%) and `memset` (2.59%) above every prosper function in the render-thread profile -- one `std::vector<std::pair<bool, FoldStateSnapshot>>` value-initialised per fold call, at **16,504 bytes** per slot | **Falsified by counting how often the branch is taken, not how big the slot is.** New `PROSPER_STAGE_FOLD_PROFILE` counters (slots reserved against captures taken) over a 400,000-call routed *Grand Theft Auto V* window: **19,513 slots, 0.0488 per call -- 95.1% of fold calls reserve none at all**. Total snapshot traffic is ~2.53% of fold time, about **0.10%** of the render thread, and an after-profile with slot pooling in place was not lower. The pooling was reverted; the counters that produced the negative are kept, so this is re-checkable rather than re-derivable. The `sizeof` is real (measured by a deliberate template-probe compile error) -- the arithmetic was right and the frequency was never measured | #3407 |
