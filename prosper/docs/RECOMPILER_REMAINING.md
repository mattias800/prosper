...
| GTA V `0x413ce6000`'s invalid **record-4 V# is live — raw selector-4 presence among the 2,064 pc70 records implies pc153 is reachable**, so the descriptor must be admitted or the dispatch rejected | **Falsified one level above the per-lane question: the whole interval is scalar-dead.** The failed 2,064-thread capture retains `user_sgprs[10] = 0`, so pc3 `S_LSHR_B32 s106,s10,2` yields zero, pc28 `S_CMP_EQ_U32 0,s106` sets SCC, pc35 `S_CBRANCH_SCC0 pc62` is **not** taken, and pc61 `S_BRANCH pc180` skips `[62,180)` — the interval containing source pc70 and consumers pc153/156/158. Those records are never read on that dispatch, so no per-lane/EXEC reachability argument is required or valid. Discovery retains the PCs only because `rdna2_specialize_shader_constant_branches()` deliberately refuses entry/user SGPRs: the compute cache key stores the user-SGPR **count**, not the values, since hashing pointer-heavy dispatch data would fragment the module cache. The repair is a bounded generic dispatch proof (#2511), not a wider title contract. | #2481, #2511 |
| A temporary two-bit `OpAtomicOr` probe emitted immediately before `0x413ce6000` pc153 **wrote `00000000`, therefore no surviving wave reaches pc153** | **Conclusion true, derivation void — and that combination is why this row exists.** pc153 genuinely is not reached, but the probe could not establish it: a zero from an opt-in *translated* diagnostic cannot distinguish "the guest never reached the site" from "the emitted diagnostic path did not execute", and the reading was withdrawn before the real cause (the scalar-dead `[62,180)` interval, row above) was found. It was briefly used to support the pc75 tag-7 predicate, which is false. **Do not delete a guest access on the strength of an instrument that reports its own non-execution the same way it reports the guest's.** Diagnose probe/emission behaviour first, then use the guest's own scalar state. | #2481 |
