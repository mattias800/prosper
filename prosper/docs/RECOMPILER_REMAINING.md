...
|---|---|---|
| Dropping the `b.is_compute` gate on the wave-mask scalar-pair projection (`rdna2_emit_alu.cpp`'s `mask()`) admits the fragment shaders it rejects | **No, and on the measured case it would replace a visible reject with silently wrong pixels.** The gate is scope-of-evidence rather than a semantic exclusion — fragment lane identity IS sound, `guest_lane_id()` returns `subgroup_local_id()` for fragment and the backend enforces an exact 64-lane fragment subgroup via `requiredSubgroupSize`, and the sibling clause `inline_int_mask_bit` already performs the identical projection in fragment. **But the stage predicate is not what is missing.** The straight-line emitter seeds absent SGPRs with a fabricated `uconst(0)` at loop phis and if-merges and then sets `scalar_presence_has_no_placeholders = false`; the dispatcher instead MUST-filters `state.sreg`, which is the proof the straight-line path lacks. `mask()`'s fallback never consults that flag. Measured on *Sonic Frontiers*' dropped HDR producer (`s_andn2_b64 s[68:69], s[84:85], EXEC` at pc=328): `sreg_bool=0 sreg=1/1 sreg_input=0/0 no_placeholders=0 stage=fragment wave=64 native_sg=0` — the words are present but unproven, so projecting them risks an all-zero survivor mask. The work is the scalar-lifetime proof, not the gate. Also settled by the same probe: the operand is **not** EXEC-lineage (`sreg_bool=0`), so "the mask was lost upstream and widening changes nothing" is false here. | #2790 |
