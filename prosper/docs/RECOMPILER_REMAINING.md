...
| GTA V `0x413ce6000`'s invalid **record-4 V# is live — raw selector-4 presence among the 2,064 pc70 records implies pc153 is reachable**, so the descriptor must be admitted or the dispatch rejected | **Falsified one level above the per-lane question: the whole interval is scalar-dead.** The failed 2,064-thread capture retains `user_sgprs[10] = 0`, so pc3 `S_LSHR_B32 s106,s10,2` yields zero, pc28 `S_CMP_EQ_U32 0,s106` sets SCC, pc35 `S_CBRANCH_SCC0 pc62` is **not** taken, and pc61 `S_BRANCH pc180` skips `[62,180)` — the interval containing source pc70 and consumers pc153/156/158. Those records are never read on that dispatch, so no per-lane/EXEC reachability argument is required or valid. Discovery retains the PCs only because `rdna2_specialize_shader_constant_branches()` deliberately refuses entry/user SGPRs, and that refusal is correct today: `ShaderCompileKey` records only the user-SGPR **count**, so a module specialized on their values could be reused under different ones. Adding every value to the key is **not** the fix — pointer-heavy dispatch data would fragment the module cache. The repair is a bounded generic dispatch proof that participates in identity (#2511), not a wider title contract. | #2481, #2511 |
