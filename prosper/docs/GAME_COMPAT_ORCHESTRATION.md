...
| 72 | **CR2/`si_addr` read as the complete span of a memory write** | A qword store beginning four bytes before the selected interval faults at that earlier address while modifying four selected bytes; start-address classification alone recorded `selected=no`. The bounded signal-safe decoder now proves contiguous spans for common MOV stores and tests interval overlap. Unrecognized opcodes with changed selected bytes are positive; unrecognized unchanged events are `selected=unknown`, because a same-value crossing store cannot be excluded. **A fault address is one byte, not an operand extent. Claim `no` only from a proved disjoint span; otherwise say unknown rather than manufacturing a negative.** #1902 / PR #1910. |
