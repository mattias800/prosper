...
| 43 | **A declared format read as a description of the content** | Dragon Quest VII's MAIN AudioOut2 port declares `data_format=0xc00` -> **12 channels**, and #1700 was specified from that number: "12 channels is a plausible real layout (7.1.4 - a 7.1 bed plus four height channels)", with the fold-down's difficulty framed as recovering the height tier's order. Measured with `PROSPER_AUDIO_LAYOUT=1` over **19,962,368 frames**, the bed is a **stereo mix in a 12-channel container**: ch0/ch1 carry everything (peak 0.407, mutual correlation +0.46), ch2 and ch4..ch7 hold a ~1e-9 residue, and **ch3 and ch8..ch11 are exactly zero for the entire run** - no LFE send, no height tier, nine dead channels. The design problem the issue posed did not exist. Two mechanisms, and the second is the one to carry away. First, a **declared** format describes the container the guest allocated, not what it wrote into it; a title may open the widest bed the platform offers and fill two channels of it, exactly as a 4-channel texture may be written with one. Second, and worse, the corroboration looked independent and was not: the `2,310,144 B/s` in the issue is `frames x channels x 4` with `channels` taken from the same decode, so the byte rate could not have disagreed with the channel count **whatever the buffer really contained** - it restates its own input. Same shape as #34: a check that could not have failed, read as confirmation. **A statistic derived from a decode cannot cross-check that decode**; you need a measurement whose value comes from somewhere else - here the per-channel content itself, and the guest's grain-pointer stride, neither of which is computed from `data_format`. The practical rule for any width/stride/format field: before designing for the declared shape, measure the payload, and note that this cuts **both** ways - the same evidence that showed nine channels empty is what confirms the two that are not, and reading `rms=0.000000` alone would have merged "exactly zero" with "1e-9 residue", which are different findings (`nonzero%` separates them). |
