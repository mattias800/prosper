...
| 89 | **A publish counter that stops, read as "the renderer failed" or "the guest stopped"** — trap 87's complementary half, on the *gate* rather than the log | `execute_ordered_and_present` publishes only when the renderer's frame is exactly `width*height*4` bytes; a **non-empty frame of a different extent was dropped with no output whatsoever**, from either side. The frontend's `[render] … Vulkan render FAILED` covers only the *empty*-frame case (trap 87), and the executor's gate said nothing at all, so a wrong-extent frame produced the same observable as a dead guest: `frame_seq` simply stops. On Sonic Frontiers (#1968) that read first as "the guest stops submitting GPU work" (the original issue title) and then, once falsified, as an open renderer question — while the actual event was prosper's renderer handing prosper's own caller a **1920x1080** frame (8,294,400 bytes) for a **3840x2160** present and the caller silently discarding it, 20,000 submits into the run. The present source is selected by target *identity* only (flipped front buffer > any registered scanout > last RGBA8 pass), with no extent contract, and the "hold the last good scanout" fallback tests `!empty()` rather than the extent, so one wrong-extent buffer overwrites the retained good frame and the wall becomes permanent. Both gates are now fail-visible: `[agc] PUBLISH DROPPED submit #N: the renderer returned B bytes for a requested WxH frame` and `[rtt] PRESENT SOURCE EXTENT MISMATCH: selected px_front\|px_vo\|px_last at GWxGH`. Before concluding anything from a stalled publish counter, check whether *any* code on the path is able to say no. #1968 |
