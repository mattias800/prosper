...
| 49 | **A tripwire whose CAP was quoted as its count, and a bookkeeping counter read as a fact about the guest** | Two errors from one hour on ArcRunner (#1226), and the second is the one that nearly got published. `PROSPER_FORGE_TRIP` printed exactly `if (n++ < 64)` lines, so "the tripwire fires **64 times**, every hit identical" — restated across sessions as a measurement — was the **limit**, indistinguishable from 64,000 hits; the neighbouring `PROSPER_MB3_FREELIST_GUARD` figure it was being compared against ("64 suppressed live-protocol writes per run") may or may not be the same population, and nothing in either log could say. **A capped diagnostic must carry its ordinal and keep printing sparsely past the cap**, or its ceiling becomes the finding. The second error: the guard beside it suppressed *none* of those writes, and the reason turned out to be `live_pair` = `dma_exec_n > rel_exec_n` — read as "the guest still owns this label, so the write is correct, so the tripwire is a false positive". Those counters are **prosper's own record of which of its writes it has performed**; they say nothing about guest ownership, and `waf_guard()`'s comment already documented the exact case that produces them on an *already-freed* block. The conclusion was one edit away from a doc entry declaring the lead dead. What settled it was making the two hypotheses predict **different observables** instead of arguing about the same ones: the forged value is a pointer with its low bit set, so print what a misaligned dereference of it would return. It returned `0x30016000` on every hit — byte-for-byte the constant the terminal fault dereferences — and an A/B that suppressed the write moved the same fault to the next link in the same walk. That collapsed two "separate legs" this issue had tracked since #1249 into one defect. **The rule: when a counter and a hypothesis are about different subjects (our bookkeeping vs. the guest's heap), no amount of the counter settles the hypothesis — construct the observable that differs.** Cousin of #46 (samples of a series read as the series) and #34 (a check that could not have failed). |
