...
| 34 | **A check of the ANSWER read as a check of the RULE** — and the unifying shape behind a third of this list | Deriving an NV12 frame size (#1687), the rule "`w*h*3/2` and `wh + (wh+1)/2` agree" was stated three times, each version confidently wrong in a smaller way: "identical for every reachable input", then "agree whenever `w*h` is even". The truth is **exact iff BOTH dimensions are even** — an even *product* is not sufficient and is actively a trap, since `1920x1081` has an even product and the derivation is **960 bytes short**. The error survived three rounds of competent review because every size anyone checked (`1920x1088`, `1920x1080`, `640x480`) is **both-even**, so each spot check confirmed the true rule *and* the wrong one equally. **The cases in front of you are not a sample; they are a coincidence until you show they discriminate.** A 40x40 exhaustive grid settled it in one command and could have been run first. The general form: verifying the *answer* on the examples you already have tells you nothing about the *rule* — you must ask what input would distinguish your rule from its near-miss, and check that one. This is the same defect as an A/B whose arms agree when they must differ (see the tell under #10), a pre-registered decision rule that cannot separate its two hypotheses, a vacuous assertion that holds whether or not the event occurred, and #33's pixel assertion on a draw that never ran. **Every one of those PASSED**, which is why none announced itself. Note where it bit: this is normally reached for around *instruments*, and here it was plain arithmetic — nobody thinks to apply the discipline to a formula. |
...
- **Two mechanical holes in the merge gate, both of which let a PR read as satisfied when it is not.**
  - **A registered review is not the same as a comment.** `gh pr comment` creates an *issue* comment;
    ...
    are right there while the PR reads as unreviewed. Related: `gh` is normally authenticated **as the
    PR author**, so `--approve` is rejected outright by GitHub — which is why comment reviews are this
    repo's convention, and the mechanical reason `reviewDecision` stays empty and must never be the
    thing checked.
  - **A rebase invalidates every review on the branch, and the gate cannot see it.** `reviews` stays
    non-empty and still reports `state=COMMENTED`/`APPROVED`, but its `commit_id` points at a SHA that
    no longer exists on the branch — so a rebased PR can satisfy a review gate with approval of code
    that is no longer there. Always compare each review's `commit_id` against the current head:
    `gh api repos/OWNER/REPO/pulls/N/reviews -q '.[] | "\(.state) \(.commit_id)"'`. The same applies
    to CI: re-check bound to the exact current SHA, never to the PR number.
