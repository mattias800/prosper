...
**4,574 refusals in a 6 s boot** (568 of `0x40`, the rest `0x4000`/`0x10000`), so **286 MiB** of pool
went with them, against the 300 MiB scratch block that is the only headroom left after the halving
probe. *Sifu* (`PPSA03001`) produces **32,192** of them, i.e. **1.96 GiB**. Fixed in
...
| The **`ampr push-map … -> FAILED`** lines are lost page commits — the guest's allocator cannot commit, so it declares OOM | Falsified 2026-08-23, **structurally**. `map_phys_at` returns null in only two situations: the address/length is not page-aligned — and a 64-byte "map" was never a page commit — or `range_is_free_reservation` said no, which at `src/hle/memory/hle_kernel_mem.cpp:1183` means the range contains a **committed** mapping or an untracked gap. So a refusal means the guest *already holds* that memory and prosper declined to `MAP_FIXED` over it, which is exactly the #88 / #107 clobber the flavor discriminator exists to prevent. Refusing is protective, not lossy, and this argument does not depend on any probe being right. **Correction, and read it before quoting the census:** this row first carried a measured "100% of the refusals target memory the guest ALREADY HAS MAPPED". That number came from a `mincore` probe of **one page** against ranges that are mostly `0x4000` — four host pages — while `MAP_FIXED_NOREPLACE` fails if *any* page in the range is mapped. The instrument examined a quarter of each range and the claim was stated for all of it, in the direction the author wanted. Caught in review of #2947, probe widened to the whole range; the re-derived numbers are in that PR and corroborate the structural argument rather than carrying it. #2908 / #2947. |
