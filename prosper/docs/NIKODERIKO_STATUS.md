...
Baseline reject inventory on unmodified `0502aaf1`, 440 s boot with `PROSPER_DBG=1
...
|---|---|---|
| The #2741 compute class — `VCC_LO`/`M0` recycled as ordinary scalar data — is what drops this title's 3D world, the way it drops UE volumetric fog in *Plucky Squire*, *Dragon Quest VII*, *The Pathless* and *Little Nightmares III* | **Falsified.** A 441 s `tools/screenshot` arm on `9ea76a52` (55/55 samples at 3840x2160, `stop=request-satisfied guest=running status=ok`) with `PROSPER_COMPUTE_PROGRAM_CENSUS=1` reports **65,536 dispatch decisions over 17 programs and ZERO programs that never execute**. The single program that skips at all, `0x300edc0000`, **executed 2,621 times against 8 skips** — the self-recovering descriptor transient of #1581 — and rejects on `image_store … dim:SQ_RSRC_IMG_2D` against `addr=0xdcd000000000 size=0`, not on anything in the `VCC`/`M0` family. No froxel-shaped resource is bound by any program. The zero is a *measured* zero, not a silent instrument: the census header printed on every block, and the same binary and harness reported 16 rejected programs on `PPSA05143` and reproduced #2741's two on `PPSA15319` at the exact pcs and words. Scope: this is a zero for the phase this route reaches (title + EULA); a froxel pass is a 3D-scene pass. | #2747 |
