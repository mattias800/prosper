...
| Sonic is black because whole graphics or compute pipelines are dropped on a recompiler reject — the *Sonic Racing: CrossWorlds* defect class (#2013/#2067, where a 16-bit VALU family rejected every draw of two pipelines and `[exec-recompile-reject]` fired 256+ times per boot) | **Falsified on current master `66eaf77b`, with the gated channel proved open in the same run.** A 60 s live-renderer arm at native 3840x2160 (`PROSPER_RENDER_SCALE=1 PROSPER_RENDER_EVERY=1`, route `long-input-probe.pad`, `PROSPER_DBG=1`) yields a full log-prefix histogram containing `[graphics-cfg]` x6 and `[compute-cfg]` x6 — both DBG-only prefixes, so the channel `[exec-recompile-reject]` shares is demonstrably live — and **zero** lines matching `recompile-reject`, `cfg-`, `exec-`, or `[compute] … skipped`. All twelve CFG lines report `cf_rejected=0`. Sonic's entire frame is six unique graphics pipelines and six unique compute pipelines, and every one of them translates. The frame stayed `distinct_rgb_colors=1 nonblack_rgb_pixels=0 crc32=666f7b3f` across all five samples. Note #2067 is *not* merged at this head, so the 16-bit VALU family is still rejected here — this arm would have caught it. | #1905, #2067, this doc |
