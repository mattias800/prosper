...
Its synthetic atlas canary fails when the exact range is erased and passes after restoration. A live
run must next establish whether that first draw actually names the atlas; even an overlap would be
evidence about the graphics resource, not yet proof that descriptor/index/shader reads during draw
realization can safely cross stale guest bytes.
