...
| Syberia's ~1.9 fps is #1748-style AGC command-buffer churn | **Falsified.** Over a 45 s headless boot with `PROSPER_DCBFULL=1` the title issues **zero** Dcb buffer-full callbacks — it never asks the guest for more command-buffer space at all, so it cannot be paying the per-submit chunk cost that drained 16 GiB on Asterix & Obelix - Babylon Mission (~466 callbacks/s). Bendy (~8 fps) and Dragon Quest VII (~12 fps) were checked in the same pass and are also clear: 11.4/s and 2.5/s, every event with the buffer genuinely exhausted. | #1756, `docs/AGC_PACKET_SIZES.md` |
