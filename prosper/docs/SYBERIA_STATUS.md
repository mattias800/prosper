...
| After owner-resolved alias accounting, the 11-step save-warning atlas reaches a guest-image, raw-buffer, DMA, ordered-memory, capture, or submit-end observer before graphics | **Falsified by the post-#1899 exact consumer census on `e8d61761`.** Across 140 frames, all 1,540 selected outputs were retained: 140 were admitted and 1,400 same-range results replaced them. All 5,600 selected image-source observations were GPU-backed (`guest=0`), including 4,200 aliases. Exactly 140 materializations remained, all at the first draw's deliberately unknown range; guest-image, raw-buffer, DMA, memory-effect, capture, unknown-consumer, and submit-end attribution were each zero. The result proves eleven outputs coalesce to one pending result per frame, but does **not** authorize removing that final writeback: the draw seam still lacks exact pre-realization coverage. | #1737 |
| The first unknown-range draw after each save-warning atlas chain actually names that atlas as a graphics resource | **Falsified by the post-realization audit on `90d8b35b`.** The existing pre-draw boundary remained fail-closed, then the realized draw reported exact live VRT/PRT guest ranges using the capture planner's conservative footprints. All 140 probes completed on realized draws: 700 valid ranges were unrelated, zero overlapped the 12.8 MiB atlas, and there were zero invalid, unrealized, superseded, or unfinished probes. Erasing the emitted byte span made exactly the named synthetic atlas canary fail. This proves the current first-draw materialization is collateral to the blanket boundary; it does not yet prove whether a **later** draw in the submit consumes the atlas. | #1737 |
| No later realized draw in the same submit consumes the retired save-warning atlas, so its final guest writeback can be deferred through all draws | **Falsified by full-submit retained-identity censuses on `b34c0a4f` and exact-current confirmation `fbd5a317`.** The existing first-draw probe again found zero overlaps in 700 exact ranges per arm, but the new probe retained the same atlas identity through every subsequent draw. All 140 valid epochs in each arm had at least one later overlap, with 700 realized overlaps total. The bounded detail sample (first 16 plus powers of two; 21 of 700 rows) consistently showed a full-size texture range at one-based draw ordinal 34 and bindings 35/38/39/40/41. It does **not** prove five overlaps per epoch, exact equality for all 700 ranges, or that draw 34 was the earliest overlap. There were zero first-draw overlaps, invalid ranges, supersessions, interruptions, or active epochs at shutdown. A defect-shaped mutation that drops the retained range after draw one is killed only by the named later-draw canary. The atlas therefore has a real later graphics consumer; do **not** remove its materialization based on the first-draw result. Pre-realization descriptor, index, shader, vertex, and indirect reads remain unclassified. | #1737 |
...
The exact-hash census on `c5bf9152` resolved the missing shape evidence (see `## Ruled out`). The
producer really uses a depth-one `Dim2D arrayed=1` storage image; each following consumer samples an
ordinary `Dim2D arrayed=0` view of the same allocation. The consumer passes every transfer gate but
finds no cache because the arrayed producer was not eligible for typed storage. The narrow generic
...
A post-#1899 authority census on exact consumer `0xee8584cf839a5b44` now proves the eleven
writebacks have only one architectural candidate per frame: 140 admissions plus 1,400 same-range
replacements, with all 5,600 sampled-image observations GPU-backed. The one remaining forced
materialization per frame is the first draw's conservative unknown-range boundary, not an observed
guest-image, raw-buffer, DMA, memory, capture, or submit-end consumer. The diagnostic therefore now
audits the subsequently realized draw-resource ranges without changing that fail-closed decision.
Its synthetic atlas canary fails when the exact range is erased and passes after restoration. The
first live audit then completed 140/140 probes with 700 exact, valid resource ranges: all 700 were
unrelated to the atlas, with zero overlap or unrealized draws. A full-submit retained-identity census
then supplied the missing counterexample: every one of 140 valid epochs found at least one later
overlap, with 700 realized overlaps total, while the first-draw probe remained at zero. All 21 bounded
detail samples showed the full-size texture range at draw ordinal 34 and bindings among
35/38/39/40/41; sparse detail does not establish that distribution for all 700 events or prove the
earliest overlapping ordinal. Deferring through the full submit is unsafe. The next discriminator
must cover the descriptor, index, shader, vertex, and indirect reads performed while realizing draws
and add population-wide ordinal/range evidence before proposing a narrower authority lifetime.

The first full-submit arm was non-timing evidence. An accidental no-game `prosper-app --help` probe
initialized Vulkan for about one second and presented zero frames before it was stopped; the planned
arm began only after a clean empty process census. Its mid-run census contained only `prosper-app`.
The post-run census contained no Prosper app but did catch another lane's `gpu_replay` at age zero,
after this app had exited. These events were disclosed to the orchestrator and do not support a
...
The cause was a **single-opcode allow-list**, `src/gpu/recompiler/rdna2_to_spirv.cpp:7100`:
