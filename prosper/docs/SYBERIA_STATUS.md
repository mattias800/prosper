...
| No later realized draw in the same submit consumes the retired save-warning atlas, so its final guest writeback can be deferred through all draws | **Falsified by full-submit retained-identity censuses on `b34c0a4f` and exact-current confirmation `fbd5a317`.** The existing first-draw probe again found zero overlaps in 700 exact ranges per arm, but the new probe retained the same atlas identity through every subsequent draw. All 140 epochs completed valid in each arm and all 140 found overlap: 700 exact texture ranges, five per epoch, at one-based draw ordinal 34 and bindings 35/38/39/40/41. There were zero first-draw overlaps, invalid ranges, supersessions, interruptions, or active epochs at shutdown. A defect-shaped mutation that clears the epoch after draw one is killed only by the named later-draw canary. The atlas therefore has a real later graphics consumer; do **not** remove or defer its materialization past draw 34. Pre-realization descriptor, index, shader, vertex, and indirect reads remain unclassified. | #1737 |
...
then supplied the missing counterexample: every one of 140 valid epochs found five exact texture
overlaps at one-based draw ordinal 34 (bindings 35/38/39/40/41), while the first-draw probe remained
at zero. The blanket boundary still forces guest visibility earlier than the demonstrated consumer,
but deferring through the full submit is unsafe. The next discriminator must cover the descriptor,
index, shader, vertex, and indirect reads performed while realizing draws, then determine whether the
atlas can remain GPU-authoritative only until the exact draw-34 consumer.
