...
| The `R11G11B10F` gap in `set_live_target_image_written_notifier` is what blacks this frame | **Falsified as the mechanism for this frame** (the *defect* is real and is fixed by #1626). A routed live A/B — same route, same timings, no diagnostics in either arm, differing only in the notifier line — produced a **byte-identical** settled menu (`md5 1fc612606b4e32adc89db9bfd35c92b2`, `crc=bdf7a33a` for every capture from t=165 s to t=220 s), and the right ~55% is still pure black. The notifier is called from exactly one site (`live_compute.cpp:6152`) under `bi.mirror_result_to_imported`, which the seed-skip path at `:3552` excludes — so it provably cannot fire for a write-only binding proved to cover every texel, which is what this dispatch's recorded `[seed-skip-verify]` line reports. | #1619 comments, PR #1626 |
| The offline localization transfers to the default render path | **Falsified.** `PROSPER_RTTLOG` and `PROSPER_RESOURCE_HASH_DIM` both clear `live_gpu_targets` (`live_renderer.cpp:933`) and force the **CPU readback** RTT path. The frame is black on both paths, but reproducing on both does **not** make it the same mechanism on both. The chain in `## Where the scene dies, exactly` is sound about the CPU-readback path only. | #1619 comments |
...
run-local, so the recorded `0x2134c14100` cannot be pre-seeded from an older run.
