...
- **The HTILE `gpu-preserving` suppression is load-bearing for GTA V's picture.** FALSE, measured
  2026-08-28 (#3089). `1b5b9471` stopped invalidating retained depth when a guest HTILE write
  ...
  **unsound in principle**: prosper never writes rendered HiZ back into the guest HTILE plane, so
  ...
  `changed=0` for a fast CLEAR exactly as readily as for the decompress it was written for. It cost
  *Blue Prince* its entire picture (every DS invalidation suppressed; 0.00% non-black over 16,500+
  readbacks). **Removing it does not cost GTA V anything measurable**: one binary, one lever, the
