...
full-colour movie frame, while replaying the same submit on unmodified `10374703` produces the
...
Recompiling every retained raw shader on `3e7cdc76` changes the exact replay hash to
...
Measured on `c9d2ef6d` with a low-volume timeline (`PROSPER_SYNCLOG=1` plus **both**
...
Three live confirmations, all on `e98a8fdb` (master `0bf229b0`) with the default CPU-only route:
...
guest, on `e98a8fdb` (master `0bf229b0`). The host was shared with 8–9 other agent lanes, so the
...
| The first state-3 renderer completion poll or `sceVideoOutLatencyControlWaitBeforeInput` should delay startup until the LOGIN pump runs | **Falsified.** Both double-buffered completion qwords are initialized to 2, and an armed breakpoint recorded zero hits at the poll while the sole downstream VideoOut call proved execution crossed it before the ordinary fault. Available implementations allow the latency-control hint to return OK immediately, so adding a guessed delay has no oracle. The remaining #1746 frontier is an unidentified generic scheduling contract, if one exists. | `eboot+0x1acb0/+0x1acca`, CPU-only `817385ef` run, #1746 |
...
| The INPUT worker is created late under prosper, so its 400 ms window starts too late | **Falsified.** `[thread] create … name=INPUT` appears at `PROSPER_SYNCLOG` line 66 of a default boot, roughly 2,150 lines before the `DLLLoadStart` dlsym that begins the load. The worker is created and asleep long before the load starts. | `PROSPER_SYNCLOG=1` default boot on `6a40d191`, #1746 |
...
## Rung 1 reached (2026-08-05, `c9d2ef6d`)
...
- **`tools/screenshot` printed `status=ok` after the guest thread had died — fixed** (#2007). On
  ...
  race**: on `555bddf2` with a warm page cache and `PROSPER_RENDER=1`, three attempts split two
...
| The renderer has no AvPlayer chroma contract on the live path (only replay lacks it) | **Falsified.** The contract exists and is reached; it *rejected* the plane. The live verdict log names the clause: `dim=5 -> not-narrow-linear-rg8`. *(That reason string is quoted as it was emitted in 2026-08-05. #2731 renamed it to `not-narrow-rg8-plane`, because the predicate now admits GPU-tiled planes too — grep the new name in `avplayer_plane_policy.hpp`.)* | `PROSPER_AVPCHROMA_LOG`, #2005 |
...
<p align="center">
  ...
  evicted with <code>tools/dropcache.py</code>. Sample 07 of 22 on <code>3225a583</code> + this fix.</sub>
</p>
...
`tools/screenshot`, default route, page cache evicted, unmodified binary and guest, on `3225a583`
...
On master `f7cfaf50` the title reaches stage 1 under the route above — `PROSPER_MODLOG=1` records
...
**It is a regression, not the title's ceiling.** At `7cc74ef5` — the #2061 merge, the head this
...
samples at t ≥ 20 s: **2,073,474 at `7cc74ef5` against 2,728 on master**, three orders of magnitude
...
From `e09a0cec` (2026-08-11) until the fix, this title rendered its publisher logo, its opening movie
...
### Bisected to `e09a0cec`, with the instrument that got it wrong first

The regression begins at **`e09a0cec`** — `fix(recompiler): preserve GTA Wave64 scalar mask words`
(2026-08-11, +363 lines in the RDNA2→SPIR-V recompiler, one of the `fix/issue-2481-*` GTA V Wave64
mask series). Its **direct parent** `a37ef264` renders on this title's own route (peak 2,004,706
non-black pixels, 11 distinct late frame CRCs); `e09a0cec` freezes (1,166,928, **1** distinct).

**What is not established:** that reverting it repairs current master. Reverting the *later*
`ade67113` hunk on master changed nothing, and at `8ae101c5` the frame is entirely black from t=2 s —
