// pm4_registers.hpp — RDNA2 / PS5 "Gen5" PM4 register offsets, field masks/shifts, and packet opcodes.
//
// VENDORED VERBATIM from Kyty (MIT License, Copyright (c) 2021 Ivan Chikhradze),
// include/Emulator/Graphics/Pm4.h. These are the hardware register dword-offsets (CB_*, DB_*, PA_*,
// SPI_*, VGT/GE_*, COMPUTE_*), their per-field *_MASK / *_SHIFT constants, and IT_* packet opcodes
// that the AGC command stream carries. Shared by the RegisterDefaults tables (agc_reg_defaults.cpp)
// and the CommandProcessor render-state extractor (render_state.cpp) so register meanings live once.
//
// Field extraction mirrors Kyty's KYTY_PM4_GET(v, REG, FIELD) = ((v) >> REG_FIELD_SHIFT) & REG_FIELD_MASK;
// use PM4_FIELD(v, REG, FIELD) below.
#pragma once
#include <cstdint>
...
constexpr uint32_t PA_SC_WINDOW_OFFSET_WINDOW_X_OFFSET_SHIFT = 0;
constexpr uint32_t PA_SC_WINDOW_OFFSET_WINDOW_X_OFFSET_MASK  = 0xFFFF;
constexpr uint32_t PA_SC_WINDOW_OFFSET_WINDOW_Y_OFFSET_SHIFT = 16;
constexpr uint32_t PA_SC_WINDOW_OFFSET_WINDOW_Y_OFFSET_MASK  = 0xFFFF;
...
constexpr uint32_t CB_BLEND0_CONTROL_DISABLE_ROP3_SHIFT         = 31;
constexpr uint32_t CB_BLEND0_CONTROL_DISABLE_ROP3_MASK          = 0x1;
...
// Color-target register blocks are 0xF context registers apart. Keep MRT1 named explicitly because
// Prosper supports the first two simultaneous attachments; the field layouts match MRT0.
constexpr uint32_t CB_COLOR1_BASE           = 0x327;
constexpr uint32_t CB_COLOR1_VIEW           = 0x32A;
constexpr uint32_t CB_COLOR1_INFO           = 0x32B;
constexpr uint32_t CB_COLOR1_ATTRIB         = 0x32C;
constexpr uint32_t CB_COLOR1_DCC_CONTROL    = 0x32D;
constexpr uint32_t CB_COLOR1_CMASK          = 0x32E;
constexpr uint32_t CB_COLOR1_FMASK          = 0x330;
constexpr uint32_t CB_COLOR1_CLEAR_WORD0    = 0x332;
constexpr uint32_t CB_COLOR1_CLEAR_WORD1    = 0x333;
constexpr uint32_t CB_COLOR1_DCC_BASE       = 0x334;
...
constexpr uint32_t CB_COLOR1_BASE_EXT       = 0x391;
...
constexpr uint32_t CB_COLOR1_ATTRIB2 = 0x3B1;
...
constexpr uint32_t COMPUTE_PGM_RSRC1_FP32_DENORM_SHIFT = 16;
constexpr uint32_t COMPUTE_PGM_RSRC1_FP32_DENORM_MASK  = 0x3;
...
// GFX10 DISPATCH_DIRECT initiator bits retained by sceAgcCbDispatch's ShaderDispatchModifier.
// USE_THREAD_DIMENSIONS switches the packet dimensions from workgroups to total threads. Clear
// CS_W32_EN selects wave64; set selects wave32.
constexpr uint32_t COMPUTE_DISPATCH_INITIATOR_USE_THREAD_DIMENSIONS_SHIFT = 5;
constexpr uint32_t COMPUTE_DISPATCH_INITIATOR_USE_THREAD_DIMENSIONS_MASK  = 0x1;
...
// Extract field FIELD from register value V of register REG (Kyty KYTY_PM4_GET equivalent).
#define PM4_FIELD(v, REG, FIELD) (((v) >> ::prosper::agc::Pm4::REG##_##FIELD##_SHIFT) & ::prosper::agc::Pm4::REG##_##FIELD##_MASK)
