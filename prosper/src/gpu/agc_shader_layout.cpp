...
// RDNA2 (GFX10/PS5) V# dword3 carries a COMBINED 7-bit FORMAT at bits [18:12] — cross-checked against
// the Kyty reference (Shader.h:585 `Format() = (fields[3] >> 12) & 0x7F` on the Gen5 path) and its
// per-value uses (74 -> R32G32B32_SFLOAT, 64 -> R32G32_SFLOAT, 77 -> 4x32 float, 56 -> 8_8_8_8 UNORM).
// The split NFMT[14:12]/DFMT[18:15] this function previously decoded is the GCN/PS4 layout: on a PS5
// V# it reads garbage fields, so every real descriptor came out DataFormat::Unknown and fell back to
// Float32 downstream — turning e.g. a UNORM8 vertex color 0xffffffff into a float32 NaN (~0 after
// clamping) and multiplying the title composite to black.
// Value list per the RDNA2 ISA buffer-format table; the four game-observed anchors (56/64/74/77) are
// Kyty-confirmed. CONFIDENCE: HIGH on anchors, MED on the rest of the table.
void rdna2_buffer_format(uint32_t fmt, DataFormat* out_fmt, uint32_t* out_components) {
    ...
    switch (fmt) {
        ...
        default: break;                                    // Unknown -> caller decides the fallback
    }
