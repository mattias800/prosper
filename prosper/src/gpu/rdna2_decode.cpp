...
uint32_t rdna2_vgpr_write_count(const Rdna2Inst& in) {
    ...
    switch (in.fmt) {
        ...
        case Rdna2Format::VOP3:
            ...
            // v_div_scale_f64, the two integer MAD64 forms, and the 64-bit reversed shifts write a
            // consecutive VGPR pair.
            return in.opcode == 0x16eu || in.opcode == 0x176u || in.opcode == 0x177u ||
                   in.opcode == kVop3OpcodeLshlrevB64 ||
                   in.opcode == kVop3OpcodeLshrrevB64 ? 2u : 1u;
        case Rdna2Format::DS:
...
uint32_t rdna2_vgpr_source_span(const Rdna2Inst& in, uint32_t source_index) {
    if (source_index >= in.n_src || in.src[source_index].kind != OperandKind::VGPR)
        return 0;
    if (in.fmt == Rdna2Format::VOP3 &&
        in.opcode == kVop3OpcodeLshrrevB64 && source_index == 1)
        return 2;
    // Dispatcher discovery historically treats every admitted DS source as a conservative four
    // dword range because read/write2 and 64/96/128-bit packets encode only the base register.
    if (in.fmt == Rdna2Format::DS) return 4;
    return 1;
}
...
bool rdna2_gta5_null_guarded_raw_store_shader(const uint32_t* code, size_t dwords) {
    // Avoid a full decode for unrelated shaders. These raw words identify the production packets;
    // decoded operand checks below remain authoritative and the direct-CFG scan closes alternate
    // entry paths that byte identity alone cannot exclude.
    ...
    for (const Rdna2Inst& in : instructions) {
        ...
        if (in.fmt == Rdna2Format::SOPK &&
            ...
            return false;
        // S_TRAP transfers to the externally configured TBA and can resume through TMA state.
        // Its destinations are not visible in this raw stream, so the exact closed-CFG proof must
        // reject it instead of assuming lexical fallthrough still dominates the guarded stores.
    ...
    // Entering pc42 itself can bypass earlier pointer dataflow even though it executes the compare.
    // Entering later can additionally bypass the mask identity, EXEC write, or EXECZ exit. A branch
    // sourced inside the proof/store interval can leave and later re-enter it. Reject all of these.
    // The debug-condition SOPP family also carries a direct simm16 target. Keep this broader check
    // feature-local: teaching the general structurizer new branch opcodes would change unrelated
    // shader emission, while this exact safety proof only needs to fail closed on alternate entry.
...
bool rdna2_gta5_null_guarded_raw_store_dispatch(
    ...
    for (const Rdna2Inst& in : instructions) {
        if (in.pc >= 42u) break;
        // These SOP1 forms add M0 to their encoded scalar destination. M0 is not part of the
        // nullable-pointer proof, so even an encoded s0 destination can dynamically overwrite
        // s2:s3 before the compare. Reject the complete GFX10.3 dynamic-SGPR-destination family.
        ...
        // Scalar ALU/VOPC/VOP3 carry destinations are at most pairs here. A base at s1 can
        // therefore overlap s2; bases s2/s3 overlap directly. SMEM may write up to sixteen words,
        // so any low destination base can cover the nullable pair. Reject conservatively instead
        // of trying to recover a value through an arbitrary pre-guard scalar computation.
