    ...
    if (version >= 59) {
        auto read_volume_views = [&](auto& bindings) {
            for (auto& binding : bindings) {
                if (!r.u32(binding.selected_mip_depth) || !r.u32(binding.first_slice) ||
        ...
        for (auto& draw : c.draws) {
            // v34 serializes slot0/1 identity in legacy named fields only. Restore that identity
            // before reading v59's volume tail, or the final alias restoration would treat every
            // valid 3D view as a stale array entry and discard it.
