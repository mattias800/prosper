...
namespace {
    ...
    LONG CALLBACK veh(EXCEPTION_POINTERS* ep) {
        ...
        if (getenv("PROSPER_VEHLOG")) {
            ...
            fprintf(stderr, "[veh] regs rax=%llx rbx=%llx rcx=%llx rdx=%llx rsi=%llx rdi=%llx "
                            "rbp=%llx rsp=%llx\n",
                    (unsigned long long)c->Rax, (unsigned long long)c->Rbx,
                    (unsigned long long)c->Rcx, (unsigned long long)c->Rdx,
                    (unsigned long long)c->Rsi, (unsigned long long)c->Rdi,
                    (unsigned long long)c->Rbp, (unsigned long long)c->Rsp);
            uint64_t bp = c->Rbp;
            for (int i = 0; i < 16 && bp > 0x10000; ++i) {
                if (!addr_readable(bp) || !addr_readable(bp + 8)) break;
                const uint64_t next = *(const uint64_t*)(uintptr_t)bp;
                const uint64_t ret = *(const uint64_t*)(uintptr_t)(bp + 8);
                fprintf(stderr, "[veh] bt#%02d bp=0x%llx ret=0x%llx\n", i,
                        (unsigned long long)bp, (unsigned long long)ret);
                if (next <= bp) break;
                bp = next;
            }
        }
