...
| --- | --- | --- |
| **Prosper's REL1 fence forge is what creates the dereferenced `0x2000000001`** — #1754's causal reading | The weak arm (`PROSPER_INIT_SUPPRESS=ptr` alone) collapsed the forge population ~200× and left the same terminal `0x30016000`, so it could not decide necessity. The combined arm named after review has now run once: the terminal signature disappears and the fault remains at the same pop with `rcx=0xd342af100024001`, which is consistent with the label writes authoring the **specific** `0x2000000001 -> 0x30016000` chain while another corruption survives. It still does not settle causality under the instrument's own contract: totals printed only at candidate 1, then not again before the sub-128 population ended, so the run lacks the required final `candidates == suppressed && landed == 0` census. Add a fault-time snapshot or dense first-64 totals before repeating; do not quote the first snapshot as the whole run. | #1226 |
