...
| **Prosper's REL1 fence forge is what creates the dereferenced `0x2000000001`** — #1754's causal reading, now retracted | `PROSPER_INIT_SUPPRESS=ptr` drops 2,048+ paired inits, which collapses the forge population from ~4,096 to **21** and additionally takes 192 fences through the existing REL1-LIVE guard. The title still faults on the **same `0x30016000`**, same pool slot, sibling pop site. Removing effectively all of prosper's label writes does not remove the corruption, so neither is *necessary* for it. | #1226 |
