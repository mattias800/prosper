...
| Suppressing the forging fence — or both known label writes — fixes the underlying allocator corruption | The fence-only arm removes the terminal `0x30016000` and moves the fault to `0x2400100024001`, two pops further along the same walk. **PROVISIONAL RUN 1** of the combined arm at `dfd89f3f`: `PROSPER_INIT_SUPPRESS=ptr` independently reached ordinal **4,096**, and `PROSPER_REL1_FORGE_SUPPRESS_ALL=1` printed its `ARMED` witness plus `candidates=1 suppressed=1 landed=0`. It reached presented frame 183, then faulted at the **same free-list pop** (`eboot+0x127e751`) with a different tagged head, `rcx=0xd342af100024001` (`addr=(nil)`). The terminal `0x30016000` signature was absent; it appears in this log only as `forge_trip`'s hypothetical pre-suppression misread. Thus this provisional arm says removing these writes is not a fix: it relocates the corruption. Apparatus limit: the forge population was at least 64 but below the next sparse detail at 128, while totals printed next at 256, so run 1 has only the first totals snapshot rather than an end-of-arm census. The three saved frames (0–2) are pixel-exact black; later nonzero-byte counts were not captured and make no visual claim. | #1226, #1754 |
...
| --- | --- | --- |
| **Prosper's REL1 fence forge is what creates the dereferenced `0x2000000001`** — #1754's causal reading | The weak arm (`PROSPER_INIT_SUPPRESS=ptr` alone) collapsed the forge population ~200× and left the same terminal `0x30016000`, so it could not decide necessity. **PROVISIONAL RUN 1** of the combined arm removed the terminal signature and left a same-pop fault with `rcx=0xd342af100024001`, consistent with the label writes authoring the **specific** `0x2000000001 -> 0x30016000` chain while another corruption survives. It does not settle causality under the instrument's own contract: totals printed only at candidate 1, then not again before the sub-128 population ended, so run 1 lacks the required final `candidates == suppressed && landed == 0` census. The diagnostic now prints every total through 256 before returning to a 256-event cadence; repeat only after its focused test and mutation gate pass. Do not quote run 1's first snapshot as the whole run. | #1226 |
